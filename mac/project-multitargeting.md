...
As estruturas de destino são especificadas no arquivo de projeto, que você pode editar clicando com o botão direito do mouse no projeto e escolhendo as **ferramentas > comando Editar arquivo** . Quando uma estrutura de destino única for especificada, use o elemento TargetFramework. O arquivo de projeto de aplicativo de console a seguir demonstra como direcionar o .NET Core 3,0:
...
Ao editar um C# arquivo em um projeto com várias estruturas de destino, você pode especificar qual estrutura de destino você deseja usar para orientar a experiência do editor (por exemplo, mostrando avisos se você usar uma API sem suporte dessa estrutura). Você pode alterar a estrutura de destino usando o seletor de **estrutura de destino** no canto superior esquerdo da janela do editor.
