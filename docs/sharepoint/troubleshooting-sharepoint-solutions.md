...
- \<*model name*> tem os seguintes erros de ativação de tipo de conteúdo externo...
...
- Administração Central do SharePoint 2010. Para obter mais informações, consulte [BDC gerenciamento de modelos](/previous-versions/office/sharepoint-server-2010/ee524073(v=office.14)#delete-a-bdc-model) no site do Microsoft TechNet.

- Windows PowerShell. Você pode excluir o modelo digitando este comando no prompt de comando: **Remove-SPBusinessDataCatalogModel**. Para obter mais informações, consulte [cmdlets gerais (SharePoint Server 2010)](/powershell/module/sharepoint-server) no site do Microsoft TechNet.
...
### <a name="resolution"></a>Resolução
 Esse erro é o resultado de colisões de ID de campo que ocorrem porque o projeto importar fluxo de trabalho reutilizável no [!INCLUDE[vsprvs](../sharepoint/includes/vsprvs-md.md)] não altera as IDs de campo de formulário de tarefa. Se você implantar um fluxo de trabalho importado no mesmo servidor que contém o fluxo de trabalho original, ocorrerão colisões de ID de campo.
...
### <a name="resolution"></a>Resolução
 ...
 Para resolver esse problema, atualize o caminho do local de implantação do arquivo de *schema.xml* no atributo CustomSchema.

## <a name="sharepoint-debugging-session-terminated-by-iis"></a>Sessão de depuração do SharePoint encerrada pelo IIS
 Esse problema ocorre se você definir um ponto de interrupção em uma [!INCLUDE[vsprvs](../sharepoint/includes/vsprvs-md.md)] solução do SharePoint, escolher a tecla **F5** para executá-lo e permanecer em um ponto de interrupção com mais de 90 segundos.
...
### <a name="resolution"></a>Resolução
 ...
 Para resolver esse problema, implante a solução por, na barra de menus, escolha **Compilar**  >  **implantar**. (Não depure a solução escolhendo a tecla **F5** .) Em seguida, exclua a instância de lista no SharePoint.
...
## <a name="error-appears-when-debugging-starts"></a>O erro aparece quando a depuração é iniciada
 Quando você começa a depurar uma solução do SharePoint no Visual Studio, um erro indica que o Visual Studio não pôde carregar o arquivo de Web.config porque a chave fornecida não estava no dicionário.

### <a name="error-message"></a>Mensagem de erro
 Não foi possível carregar o arquivo de configuração de Web.config. Verifique o arquivo em busca de qualquer elemento XML malformado e tente novamente. Ocorreu o seguinte erro: a chave fornecida não estava presente no dicionário.
