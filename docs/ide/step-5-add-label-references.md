...
1. Adicione referências de rótulo ao seu formulário usando o código a seguir.
     ...
     Quando uma variável não mantém o controle de um objeto, ela é definida como um valor reservado especial: `null` C# em e `Nothing` em Visual Basic. Desse modo, quando o programa for iniciado, `firstClicked` e `secondClicked` serão definidas como `null` ou `Nothing`, o que significa que as variáveis não estão rastreando nada.
