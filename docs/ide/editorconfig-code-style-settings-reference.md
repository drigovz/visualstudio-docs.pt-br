...
> [!TIP]
> - Ao definir as convenções de codificação em um arquivo. editorconfig, você está configurando como deseja que os [analisadores de estilo de código](../code-quality/roslyn-analyzers-overview.md) criados no Visual Studio analisem seu código. O arquivo de configuração para esses analisadores é o editorconfig.
> - As preferências de estilo de código do Visual Studio também podem ser definidas na caixa de diálogo [Opções do editor de texto](code-styles-and-code-cleanup.md). No entanto, as configurações .editorconfig têm precedência, e as preferências definidas em **Opções** não estão associadas a um projeto específico.

O final deste artigo contém um [exemplo do arquivo .editorconfig](#example-editorconfig-file).
...
- Quando essa regra está definida como **true**, dá preferência a propriedades automáticas em vez de propriedades com campos de suporte particulares.
- Quando essa regra está definida como **false**, dá preferência a propriedades com campos de suporte particulares em vez de propriedades automáticas.
...
- Quando essa regra é definida como **true**, coloque uma linha em branco entre os grupos de diretivas using.
- Quando essa regra é definida como **false**, não coloque uma linha em branco entre os grupos de diretivas using.
...
- Quando essa regra está definida como **true**, exige que membros dos inicializadores de objeto estejam em linhas separadas.
