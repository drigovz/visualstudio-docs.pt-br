...
Para desenvolver com o F# no Visual Studio, primeiro instale a carga de trabalho **desenvolvimento para desktop do .NET**, caso ainda não tenha feito isso. Você instala cargas de trabalho do Visual Studio através do Visual Studio Installer, que você pode abrir selecionando **Ferramentas** > **Obter Ferramentas e Recursos**.
...
|[Encontrar e substituir texto](finding-and-replacing-text.md)|Permite que você faça pesquisas em um arquivo, um projeto ou uma solução e, potencialmente, altere o texto.|Sim|
...
|**Clique em Ctrl**+**click** para Ir para Definição|Permite que você mantenha a tecla **Ctrl** pressionada e clique em um símbolo do F# para invocar a opção Ir para Definição.|Sim|
...
|Ir para Todos|Permite navegação global e difusa para todas as construções F# via **Ctrl**+**T**.|Sim|
...
|Conclusão automática|Quando ativado, faz com que a conclusão da palavra selecione a primeira correspondência como você digita, em vez de esperar que você selecione uma ou **pressione Ctrl**+**Space**.|Sim|
