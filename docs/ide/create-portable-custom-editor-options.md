...
Você pode adicionar um arquivo [EditorConfig](https://editorconfig.org/) ao seu projeto ou base de código para impor estilos de codificação consistentes para todos que trabalham na base de código. As configurações de EditorConfig têm precedência sobre as configurações do editor de texto global do Visual Studio. Isso significa que é possível personalizar cada base de código para usar as configurações de editor de texto específicas para esse projeto. Você ainda pode definir suas preferências pessoais editor na caixa de diálogo **Opções** do Visual Studio. Essas configurações se aplicam sempre que você está trabalhando em uma base de código sem um arquivo *.editorconfig* ou quando o arquivo *.editorconfig* não anula uma configuração específica. Um exemplo de como essa preferência é o estilo de recuo&mdash;tabulações ou espaços.
...
Quando você adiciona um arquivo EditorConfig ao seu projeto no Visual Studio, novas linhas de código são formatadas de acordo com as configurações do EditorConfig. A formatação do código existente não é alterada a menos que você execute um dos seguintes comandos:

 - [Code Cleanup](../ide/code-styles-and-code-cleanup.md) (**Ctrl**+**K**, **Ctrl**+**E**), que aplica quaisquer configurações de espaço em branco, `using` como estilo de recuo, e configurações de estilo de código selecionadas, como como classificar diretivas.
 - **Editar** > **documento de formato** **avançado** > (ou **Ctrl**+**K**, **Ctrl**+**D** no perfil padrão), que só aplica configurações de espaço em branco, como estilo de recuo.

 ::: moniker-end

::: moniker range="=vs-2017"

Quando você adiciona um arquivo EditorConfig ao seu projeto no Visual Studio, novas linhas de código são formatadas de acordo com as configurações do EditorConfig. A formatação do código existente não é alterada a menos que você execute o documento **(Editar** > **documento de formato** **avançado** > ou **Ctrl**+**K**, **Ctrl**+**D** no perfil padrão). A formatação do documento afeta apenas as configurações do espaço em branco, como o estilo indent, a menos que você tenha configurado documento de formato para realizar a [limpeza de código adicional](../ide/code-styles-and-code-cleanup.md#apply-code-styles).
...
As configurações em arquivos EditorConfig permitem manter configurações e estilos de codificação consistentes em uma base de código, como estilo de recuo, largura de tabulação, caracteres de fim de linha, codificação e mais, independentemente do editor ou IDE usado. Por exemplo, quando a codificação em C#, se sua base de código tem uma convenção para preferir que os recuos sempre consistam em cinco caracteres espaciais, os documentos usam codificação UTF-8, e cada linha sempre termina com um CR/LF, você pode configurar um arquivo *.editorconfig* para fazer isso.
...
Quando você adiciona um arquivo EditorConfig ao seu projeto ou base de código, as novas linhas de código que você escreve são formatadas de acordo com o arquivo EditorConfig. No entanto, a adição de um arquivo EditorConfig não converte estilos existentes para os novos até que você forme o documento ou execute [a Limpeza de Código](../ide/code-styles-and-code-cleanup.md). Por exemplo, se você tiver recuos em seu arquivo formatados com tabulações e adicionar um arquivo EditorConfig com recuos com espaços, os caracteres de recuo não serão convertidos automaticamente em espaços. Quando você formatar o documento (**Editar** > **documento de formato** **avançado** > ou **Ctrl**+**K**, **Ctrl**+**D**), as configurações de espaço em branco no arquivo EditorConfig são aplicadas às linhas de código existentes.
...
1. Abra um projeto ou uma solução no Visual Studio. Selecione o nó de projeto ou solução, dependendo se as configurações *do .editorconfig* devem ser aplicadas a todos os projetos da solução ou apenas a um. Você também pode selecionar uma pasta em seu projeto ou solução para adicionar o arquivo *.editorconfig.*

1. Na barra de menu, escolha **Projeto** > **Adicionar novo item**ou **pressione Ctrl**+**Shift**+**A**.
...
1. Selecione o modelo **Arquivo editorconfig (padrão)** para adicionar um arquivo EditorConfig pré-preenchido com duas opções principais do EditorConfig para tamanho e estilo de recuo. Ou selecione o modelo **Arquivo editorconfig (.NET)** para adicionar um arquivo EditorConfig pré-preenchido com [convenções de nomenclatura, formatação e estilo de código .NET](../ide/editorconfig-code-style-settings-reference.md).
   ...
   Um arquivo *.editorconfig* é exibido no Solution Explorer e é aberto no editor.
...
Para substituir algumas ou todas as configurações do EditorConfig, adicione um arquivo *.editorconfig* no nível da hierarquia de arquivos que deseja que essas configurações substituídas se apliquem. As novas configurações de arquivo do EditorConfig aplicam-se aos arquivos no mesmo nível e a todas as subpastas.

![Hierarquia do EditorConfig](../ide/media/vside_editorconfig_hierarchy.png)

Se você quiser substituir algumas, mas não todas as configurações, especifique apenas essas configurações no arquivo *.editorconfig.* Somente as propriedades que você listar explicitamente no arquivo de nível inferior são substituídas. Outras configurações de arquivos *.editorconfig* de nível superior continuam a ser aplicadas. Se você quiser garantir que _nenhuma_ configuração de _nenhum_ arquivo *.editorconfig* de nível superior ```root=true``` seja aplicada a esta parte da base de código, adicione a propriedade ao arquivo *.editorconfig* de nível inferior:
...
Se você editar vários arquivos *.editorconfig,* poderá achar útil a [extensão do Serviço de Linguagem EditorConfig.](https://marketplace.visualstudio.com/items?itemName=MadsKristensen.EditorConfig) Alguns dos recursos dessa extensão incluem realce de sintaxe, IntelliSense aprimorado, validação e formatação de código.
...
Isso significa que, se quaisquer configurações do editor no **Editor** > **de opções** > **de** ferramentas (como tamanho e estilo de recuo, tamanho da guia ou convenções de codificação) forem especificadas em um arquivo EditorConfig na estrutura do diretório, as convenções no arquivo EditorConfig anulam as configurações em **Opções**. Você pode controlar esse comportamento ativando/desativando a opção **Seguir as convenções de codificação do projeto** em **Ferramentas** > **Opções** > **Editor de Texto**. Desmarcar a opção desliga o suporte do EditorConfig para Visual Studio.
...
Você pode controlar o escopo de suas convenções editorConfig definindo a ```root=true``` propriedade no arquivo *.editorconfig* na raiz do seu repo ou no diretório em que seu projeto reside. O Visual Studio procura um arquivo chamado *.editorconfig* no diretório do arquivo aberto e em todos os diretórios principais. A pesquisa termina quando atinge o filepath raiz ou se ```root=true``` um arquivo *.editorconfig* com for encontrado.
