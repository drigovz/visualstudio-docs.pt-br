...
- Quando um usuário escolhe esse botão, o aplicativo abre uma caixa de <xref:System.Windows.Forms.OpenFileDialog>.

- Se um usuário abrir um arquivo de imagem, o aplicativo mostrará a imagem na <xref:System.Windows.Forms.PictureBox>.
...
1. Acesse o **Designer de Formulários do Windows** e clique duas vezes no botão **Mostrar uma imagem**. O IDE vai imediatamente para o designer de código e move o cursor para que ele fique dentro do método `showButton_Click()` (opcionalmente, `ShowButton_Click()`) que você adicionou anteriormente.
...
Em seguida, use o IntelliSense para inserir mais código para abrir uma caixa de diálogo **Abrir Arquivo**. Se o usuário escolheu o botão **OK**, a PictureBox carrega o arquivo selecionado pelo usuário. As etapas a seguir mostram como inserir o código e, embora existam muitas etapas, são apenas alguns pressionamentos de teclas:
 ...
 1. Adicionar outro espaço. Assim que fizer isso, outra janela do **IntelliSense** será aberta. Comece digitando `DialogResult` e pressione a tecla **Tab** para adicioná-lo.
    ...
    > [!NOTE]
    > Quando você escreve um código para chamar um método, ele às vezes retorna um valor. Nesse caso, o método **do componente**OpenFileDialog<xref:System.Windows.Forms.CommonDialog.ShowDialog> retorna um valor <xref:System.Windows.Forms.DialogResult>. DialogResult é um valor especial que indica o que acontecem em uma caixa de diálogo. Um componente **OpenFileDialog** pode resultar em o usuário escolher **OK** ou **Cancelar**. Portanto, seu método `ShowDialog()` retorna `DialogResult.OK` ou `DialogResult.Cancel`.
