...
#### <a name="parameters"></a>Parâmetros
 `name` o nome do campo ou membro. Quando o elemento `<field>` é usado em uma função de construtor, `name` é necessário e define o membro ao qual a marca se aplica. Quando o elemento `<field>` está anotando diretamente um campo, esse atributo é ignorado e o nome usado pelo Visual Studio é o nome do campo real no código-fonte.
...
- Uma função de construtor JavaScript.
  ...
  `value` Opcional. Especifica o código que deve ser avaliado para uso pelo IntelliSense em vez do próprio código de função. Por `<field>`, esse atributo tem suporte para funções de construtor, mas não tem suporte para literais de objeto. Você pode usar esse atributo para fornecer informações de tipo quando o tipo de campo for indefinido. Por exemplo, você pode usar `value=’1’` para tratar o tipo de campo como um número.

  `description` Opcional. Uma descrição para o campo.

## <a name="remarks"></a>Comentários
 O atributo `name` é necessário quando você está documentando um campo em uma função de construtor. Para todos os outros cenários, todos os atributos para o elemento `<field>` são opcionais.

 Quando você está documentando uma função de construtor, o elemento `<field>` deve aparecer imediatamente antes da declaração de campo. O atributo `name` deve corresponder ao nome do campo usado no código-fonte. Para membros de objeto, o atributo `name` pode ser omitido se o elemento `<field>` aparecer imediatamente antes da declaração de membro de objeto.
...
## <a name="example"></a>Exemplo
 O exemplo a seguir mostra como usar o elemento `<field>` com o atributo `static` definido como `true`.
...
## <a name="example"></a>Exemplo
 O exemplo a seguir mostra como usar o elemento `<field>` com o atributo `static` definido como `false`.
...
## <a name="example"></a>Exemplo
 O exemplo a seguir mostra como usar o elemento `<field>` com o atributo `value`.
