 ...
 **Edite o arquivo de modelo em um diagrama ou formulário.** [!INCLUDE[dsl](../includes/dsl-md.md)] fornece ferramentas que permitem editar um modelo como um diagrama ou formulário do Windows. Isso facilita discutir o modelo com os usuários do aplicativo gerado. [!INCLUDE[dsl](../includes/dsl-md.md)] também cria um conjunto de classes fortemente tipado que reflete a estrutura do modelo. Para obter mais informações, consulte [código de geração de uma linguagem específica do domínio](../modeling/generating-code-from-a-domain-specific-language.md).  
  
 **Usar um modelo UML**. Você pode gerar o código a partir de um modelo UML. Com isso, você tem a vantagem de que o modelo pode ser editado como um diagrama em uma notação familiar. Além disso, não é necessário projetar o diagrama. Para obter mais informações, consulte [gerar arquivos de um modelo UML](../modeling/generate-files-from-a-uml-model.md).  
