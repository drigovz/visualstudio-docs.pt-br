...
> Se você aplicar os estereótipos de um perfil em um modelo que você está editando e, em seguida, compartilhar o modelo com outras pessoas, eles deverão instalar o mesmo perfil em seus próprios computadores. Caso contrário, eles não poderão ver os estereótipos que você usou.
 ...
 Um perfil geralmente faz parte de uma [!INCLUDE[vs_current_short](../includes/vs-current-short-md.md)] extensão maior. Por exemplo, você pode definir um comando que traduz algumas partes de um modelo para codificar. Você pode definir um perfil que os usuários devem aplicar aos pacotes que desejam converter. Você distribuiria o novo comando junto com o perfil em uma única [!INCLUDE[vs_current_short](../includes/vs-current-short-md.md)] extensão.
...
## <a name="how-to-define-a-profile"></a><a name="DefineProfile"></a> Como definir um perfil
...
3. Adicione o perfil a uma extensão do Visual Studio ( `.vsix` arquivo). Você pode criar uma nova extensão para seu perfil ou adicionar o perfil a uma extensão existente.
...
6. Envie o arquivo de extensão a outros usuários do [!INCLUDE[vsprvs](../includes/vsprvs-md.md)] para instalar o em seus computadores.

## <a name="how-to-add-a-profile-to-a-visual-studio-extension"></a><a name="AddProfile"></a> Como adicionar um perfil a uma extensão do Visual Studio
 Para instalar um perfil e permitir que você o envie a outros usuários, você deve adicionar o perfil a uma extensão do Visual Studio. Para obter mais informações, consulte [implantando extensões do Visual Studio](https://msdn.microsoft.com/library/dd393694(VS.100).aspx).
...
1. Crie um projeto de extensão do Visual Studio.
   ...
   > [!NOTE]
   > Você deve ter [!INCLUDE[vsipsdk](../includes/vsipsdk-md.md)] o instalado para usar este procedimento.

   1. No menu **Arquivo** , aponte para **Novo**e clique em **Projeto**.

   2. Na caixa de diálogo **novo projeto** , em **modelos instalados**, expanda **Visual C#**, clique em **extensibilidade**e, em seguida, clique em **projeto VSIX**. Defina o nome do projeto e clique em **OK**.
...
5. Na página **ativos** , adicione uma linha que descreve o perfil:
   ...
   - Clique em **Nova**. Defina os campos na caixa de diálogo **Adicionar novo ativo** da seguinte maneira.

   - Definir **tipo** como `Microsoft.VisualStudio.UmlProfile`
...
8. **Para extrair o VSIX para implantação**
   ...
   1. No Windows Explorer, abra a pasta **.\bin\Debug** ou **.\bin\Release** para localizar o arquivo **. vsix** . Este é um [!INCLUDE[vs_current_short](../includes/vs-current-short-md.md)] arquivo de extensão. Ele pode ser instalado em seu computador e enviado a outros usuários do Visual Studio.

   2. Para instalar a extensão:

       1. Clique duas vezes no `.vsix` arquivo. O instalador de extensão do Visual Studio será iniciado.

       2. Reinicie todas as instâncias do Visual Studio em execução.

   O procedimento alternativo a seguir pode ser usado para extensões pequenas se você não tiver instalado o [!INCLUDE[vsipsdk](../includes/vsipsdk-md.md)] .
...
3. Copie um existente `extension.vsixmanifest` e edite-o com um editor de XML. Altere a ID, o nome e os nós de conteúdo.
...
6. Para instalar o perfil em qualquer computador com as edições apropriadas do Visual Studio, clique duas vezes no `.vsix` arquivo.
...
1. Clique duas vezes no `.vsix` arquivo no Windows Explorer ou abra-o no Visual Studio.
...
## <a name="how-to-define-localized-profiles"></a><a name="Localized"></a> Como definir perfis localizados
...
3. Altere o nome do novo diretório para o código abreviado ISO para a cultura localizada, como `bg` para búlgaro ou `fr` para o francês. Você deve usar um código de cultura neutro, normalmente duas letras, não uma cultura específica, como `fr-CA` . Para obter mais informações sobre códigos de cultura, consulte o [método CultureInfo. Getculturals](https://msdn.microsoft.com/library/system.globalization.cultureinfo.getcultures(VS.100).aspx), que fornece uma lista completa de códigos de cultura.

4. Adicione uma cópia do seu perfil padrão ao novo diretório. Não altere seu nome de arquivo.

     Uma pasta de extensão de exemplo [!INCLUDE[vs_current_short](../includes/vs-current-short-md.md)] , antes de ser compilada ou compactada em um `.vsix` arquivo, conterá as seguintes pastas e arquivos:
...
5. Edite a nova cópia do perfil, convertendo para o idioma de destino todas as partes que serão visíveis para o usuário, como os `displayName` atributos.
...
## <a name="the-structure-of-a-profile"></a><a name="Schema"></a> A estrutura de um perfil

 Para ajudá-lo a editar os arquivos de perfil, instale o `.xsd` arquivo em:

 **%ProgramFiles%\Microsoft Visual Studio [versão] \Xml\Schemas**

 Esta seção usa o perfil C# como exemplo. A definição completa do perfil pode ser vista em:
...
> [!NOTE]
> O atributo chamado `name` não deve conter espaços nem Pontuação. O atributo `displayName` , que aparece na interface do usuário, deve ser uma cadeia de caracteres XML válida.
...
#### <a name="property-types"></a>Tipos de propriedade
 A `<propertyTypes>` seção declara uma lista de tipos que são usados para propriedades na `<stereotypes>` seção. Há dois tipos de tipos de propriedade: external e Enumeration.
...
#### <a name="metaclasses"></a>Metaclasses
 A `<metaclasses>` seção é uma lista de tipos de elementos de modelo para os quais os estereótipos neste perfil podem ser definidos:
...
```
 ...
 O `name` atributo de `metaclassMoniker` é um link para um elemento na `<metaClasses>` seção.

> [!NOTE]
> O nome do moniker deve começar com `/yourProfileName/` , onde `yourProfileName` é definido no `name` atributo do perfil ("csharpprofile" neste exemplo). O moniker termina com o nome de uma das entradas na seção metaclasses.

 Cada estereótipo pode listar zero ou mais propriedades que ele adiciona a qualquer elemento de modelo ao qual ele é aplicado. O `<propertyType>` contém um link para um dos tipos definidos na `<propertyTypes>` seção. O link deve ser um `<externalTypeMoniker>` para fazer referência a um `<externalType>,` ou `<enumerationTypeMoniker>` a para fazer referência a um `<enumerationType>` . Novamente, o link começa com o nome do seu perfil.
...
4. Desinstale a extensão com falha. Clique em **Extensões e Atualizações** no menu **Ferramentas**.
...
5. Recompile o arquivo VSIX e abra-o no Windows Explorer para reinstalá-lo. Reinicie o [!INCLUDE[vsprvs](../includes/vsprvs-md.md)].
   ...
   1. Remover o arquivo de extensão de uma subpasta de *LocalAppData*\Microsoft\VisualStudio \\ [Version] \Extensions\
   ...
   - *LocalAppData* normalmente está em C:\Users \\ *username*\AppData\Local\
