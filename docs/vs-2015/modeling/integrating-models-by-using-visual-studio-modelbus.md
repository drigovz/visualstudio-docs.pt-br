---
title: Integrando modelos usando o ModelBus | Microsoft Docs
ms.date: 11/15/2016
...
[!INCLUDE[vsprvs](../includes/vsprvs-md.md)] O ModelBus fornece um método para criar links entre modelos e de outras ferramentas em modelos. Por exemplo, é possível vincular modelos de linguagem específica de domínio (DSL) e modelos UML. É possível criar um conjunto integrado de DSLs.
...
1. Faça o download e instale a extensão Visual Studio Model Bus, a menos que já estiver instalada. Para obter mais informações, consulte [visualização e SDK de modelagem](https://www.visualstudio.com/).
...
### <a name="making-sure-that-elements-can-be-referenced"></a>Garantindo que elementos possam ser referenciados
 [!INCLUDE[vsprvs](../includes/vsprvs-md.md)] Os adaptadores ModelBus usam o GUID de um elemento para identificá-lo, por padrão. Esses identificadores devem, portanto, ser persistidos no arquivo do modelo.
...
3. Em Depuração de projeto na instância experimental do [!INCLUDE[vsprvs](../includes/vsprvs-md.md)], inclua arquivos que são instâncias de cada DSL.
    ...
    > [!NOTE]
    > [!INCLUDE[vsprvs](../includes/vsprvs-md.md)] ModelBus só pode resolver referências a modelos que são itens na mesma [!INCLUDE[vsprvs](../includes/vsprvs-md.md)] solução. Por exemplo, não é possível criar uma referência para um arquivo de modelo em outra parte do sistema de arquivos.
