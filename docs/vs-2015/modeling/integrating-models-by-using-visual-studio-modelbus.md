...
[!INCLUDE[vsprvs](../includes/vsprvs-md.md)] ModelBus fornece um método para criar links entre modelos e de outras ferramentas em modelos. Por exemplo, você pode vincular modelos de linguagem específica do domínio (DSL) e modelos UML. É possível criar um conjunto integrado de DSLs.
...
1. Faça o download e instale a extensão Visual Studio Model Bus, a menos que já estiver instalada. Para obter mais informações, consulte [SDK de visualização e modelagem](https://go.microsoft.com/fwlink/?LinkID=185579).
...
### <a name="making-sure-that-elements-can-be-referenced"></a>Garantindo que elementos possam ser referenciados
 [!INCLUDE[vsprvs](../includes/vsprvs-md.md)] adaptadores ModelBus usam o GUID de um elemento para identificá-lo, por padrão. Esses identificadores devem, portanto, ser persistidos no arquivo do modelo.
...
2. No DSL Explorer, expanda **Comportamento de Serialização de Xml** e **Dados da Classe**.
...
1. A propriedade de domínio com o botão direito e, em seguida, clique em **propriedades específicas da ModelBusReference editar**. Uma caixa de diálogo é exibida. Este é o *Seletor do Model Bus*.

2. Selecione o **Tipo de ModelBusReference** adequado: para um modelo ou para um elemento dentro de um modelo.
...
3. Em Depuração de projeto na instância experimental do [!INCLUDE[vsprvs](../includes/vsprvs-md.md)], inclua arquivos que são instâncias de cada DSL.
    ...
    > [!NOTE]
    > [!INCLUDE[vsprvs](../includes/vsprvs-md.md)] ModelBus só pode resolver referências a modelos que são itens na mesma solução de [!INCLUDE[vsprvs](../includes/vsprvs-md.md)]. Por exemplo, não é possível criar uma referência para um arquivo de modelo em outra parte do sistema de arquivos.
...
## <a name="actions-performed-by-the-modelbus-extension"></a>Ações realizadas pela Extensão do ModelBus
 ...
 Ao clicar com o botão direito no diagrama da Definição de DSL, clique em **Habilitar Modelbus** e selecione **Habilitar esta DSL para consumir o ModelBus**:
