---
title: Personalizando o comportamento de exclusão | Microsoft Docs
ms.date: 11/15/2016
...
A exclusão de um elemento geralmente provoca também a exclusão de seus elementos relacionados. Todas as relações conectadas a ele e quaisquer elementos filhos são excluídos. Esse comportamento é chamado de *exclusão de propagação*. Você pode personalizar a propagação da exclusão, por exemplo, para providenciar que os elementos adicionais relacionados sejam excluídos. Ao escrever o código do programa, você pode fazer com que a propagação de exclusão dependa do estado do modelo. Também é possível causar outras alterações em resposta a uma exclusão.
...
- [Usando OnDelete e OnDeleted](#ondeleting) – use esses métodos em que a resposta pode incluir outras ações, como atualizar um valor dentro ou fora da loja.
...
- Por padrão, a exclusão não se propaga ao longo das relações de referência. Se houver uma relação de referência **ArtistPlaysOnAlbum** do **álbum** ao **artista**, a exclusão de um álbum não excluirá nenhum artista relacionado, e a exclusão de um artista não excluirá nenhum álbum.
...
1. No diagrama de definição de DSL, selecione a *função* para a qual você deseja que a propagação seja excluída. A função é representada pela linha à esquerda ou à direita de uma caixa de relação de domínio.
...
2. Na janela Propriedades, defina a propriedade de **exclusão Propagas** .
...
3. Pressione F5 e verifique se:
   ...
   Você também pode ver a opção **propagar exclusão** na janela **detalhes de DSL** . Selecione uma classe de domínio e, na janela detalhes de DSL, abra a página **excluir comportamento** clicando no botão no lado da janela. A opção **propagar** é mostrada para a função oposta de cada relação. A coluna **Excluir estilo** indica se a opção **propagar** está em sua configuração padrão, mas não tem nenhum efeito separado.
...
- **VisitorFilterResult. DoNotCare** – o elemento não deve ser excluído, a menos que outra consulta responda que ele deve ser excluído.

- **VisitorFilterResult. Never** – o elemento não deve ser excluído, mesmo que outra consulta responda **Sim**, e o Walker não deve tentar os outros links do elemento.
...
1. As regras de **exclusão** **e exclusão são** disparadas apenas em uma transação e não em um desfazer ou refazer. Você pode configurá-las para serem colocadas em fila para execução no final da transação em que a exclusão é realizada. As regras Deleting são sempre executadas antes de qualquer regra Deleted na fila.
     ...
     Para obter mais informações, consulte [regras propagar alterações no modelo](../modeling/rules-propagate-changes-within-the-model.md).

2. O evento Store **excluído** é invocado no final de uma transação e é chamado após um desfazer ou refazer. Ele pode, portanto, ser usado para propagar exclusões a objetos fora do repositório, tais como arquivos, entradas de banco de dados ou outros objetos no [!INCLUDE[vsprvs](../includes/vsprvs-md.md)].

     Para obter mais informações, consulte [manipuladores de eventos propagar alterações fora do modelo](../modeling/event-handlers-propagate-changes-outside-the-model.md).
