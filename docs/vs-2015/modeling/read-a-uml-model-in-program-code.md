...
## <a name="Reading"></a>Lendo um modelo no código do programa
 Para acessar o conteúdo de um modelo sem mostrá-lo em uma janela [!INCLUDE[vsprvs](../includes/vsprvs-md.md)], use `ModelingProject.LoadReadOnly()`.
...
## <a name="alternative-methods"></a>Métodos alternativos
 Para muitos aplicativos, [!INCLUDE[vsprvs](../includes/vsprvs-md.md)] ModelBus permite que você referencie modelos e elementos dentro deles, com maior robustez e flexibilidade do que com os métodos descritos neste tópico. Ele fornece um método padrão para fazer links entre elementos arbitrários, seja na mesma ou em modelos diferentes. Para obter mais informações, consulte [integrar modelos UML com outros modelos e ferramentas](../modeling/integrate-uml-models-with-other-models-and-tools.md).

 Você também pode abrir modelos e diagramas na interface do usuário usando a API [!INCLUDE[vsprvs](../includes/vsprvs-md.md)]. Para obter mais informações, consulte [abrir um modelo UML usando a API do Visual Studio](../modeling/open-a-uml-model-by-using-the-visual-studio-api.md).

## <a name="Standalone"></a>Aplicativos autônomos
 O exemplo na seção anterior funcionará nas extensões do Visual Studio. É possível ler um modelo em um aplicativo autônomo, mas você deve adicionar algumas referências ao seu projeto [!INCLUDE[vsprvs](../includes/vsprvs-md.md)].
...
2. Adicione as referências de [!INCLUDE[TLA2#tla_net](../includes/tla2sharptla-net-md.md)] necessárias para acessar modelos UML, normalmente:
...
3. Além das referências listadas nas seções anteriores, adicione as seguintes referências de projeto em **\Program Files\Microsoft Visual Studio [Version] \Common7\IDE\PrivateAssemblies**:
   ...
   - Microsoft. VisualStudio. TeamArchitect. Repositóriodemodelos. DSL. dll
   ...
   - Microsoft. VisualStudio. TeamArchitect. Designerdesequência. DSL. dll
