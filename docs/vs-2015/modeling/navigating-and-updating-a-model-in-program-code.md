...
## <a name="in-this-topic"></a>Neste tópico
 ...
 Formas, conectores e diagramas e suas relações com elementos de modelo são descritos em um tópico separado. Para obter mais informações, confira [Como: Navegue e atualize um diagrama](../misc/how-to-navigate-and-update-a-diagram.md).
...
### <a name="relationships"></a>Relações
 ...
 As propriedades em extremidades opostas de uma relação são sempre recíprocas. Quando um link é criado ou excluído, as propriedades de função em ambos os elementos são atualizadas. A expressão a seguir (que usa as extensões `System.Linq`de) é sempre verdadeira para a relação ParentsHaveChildren no exemplo:
 ...
 **ElementLinks**. Uma relação também é representada por um elemento de modelo chamado de *link*, que é uma instância do tipo de relação de domínio. Um link sempre tem um elemento de origem e um elemento de destino. O elemento de origem e o elemento de destino podem ser iguais.
 ...
 Por padrão, não é permitido que mais de uma instância de uma relação vincule qualquer par de elementos de modelo. Mas se, na definição de DSL, `Allow Duplicates` o sinalizador for verdadeiro para a relação, poderá haver mais de um link e você deverá usar: `GetLinks`
 ...
 **Funções ocultas.** Se, na definição de DSL, a **Propriedade gerada** for **false** para uma função específica, nenhuma propriedade será gerada que corresponda a essa função. No entanto, você ainda pode acessar os links e percorrer os links usando os métodos da relação:
 ...
 O exemplo usado com mais frequência é <xref:Microsoft.VisualStudio.Modeling.Diagrams.PresentationViewsSubject> a relação, que vincula um elemento de modelo à forma que o exibe em um diagrama:
...
## <a name="metadata"></a>Acessando informações de classe
...
```
 ...
 Para tornar as alterações permanentes, você `Commit` deve fazer a transação antes que ela seja descartada. Se ocorrer uma exceção que não é detectada dentro da transação, o repositório será redefinido para seu estado antes das alterações.

## <a name="elements"></a>Criando elementos de modelo
...
- Defina as propriedades de um novo elemento, especialmente a propriedade para a `IsName` qual é verdadeira no DslDefinition. Esse sinalizador marca a propriedade que serve para identificar o elemento exclusivamente dentro de seu proprietário. Nesse caso, a propriedade Name tem esse sinalizador.

- A definição de DSL dessa DSL deve ter sido carregada no repositório. Se você estiver escrevendo uma extensão como um comando de menu, isso normalmente já será verdadeiro. Em outros casos, você pode carregar explicitamente o modelo na loja ou usar [ModelBus](/previous-versions/ee904639(v=vs.140)) para carregá-lo. Para obter mais informações, confira [Como: Abra um modelo do arquivo no código](../modeling/how-to-open-a-model-from-file-in-program-code.md)do programa.
...
## <a name="links"></a>Criando links de relacionamento
...
- Os links de relação de e para o elemento. Por exemplo, `edward.Parents` não conterá `henry`mais.

- Elementos em funções para os quais `PropagatesDelete` o sinalizador é verdadeiro. Por exemplo, a forma que exibe o elemento será excluída.

  Por padrão, cada relação incorporada tem `PropagatesDelete` true na função de destino. Excluir não exclui o `familyTree`, mas `familyTree.Delete()` exclui todos os `Persons`. `henry` Para obter mais informações, consulte Personalizando o [comportamento de exclusão](../modeling/customizing-deletion-behavior.md).
  ...
  (Esse exemplo específico não teria nenhum efeito, porque `PropagatesDelete` é `false` `ParentsHaveChildren` para as funções da relação.)
...
## <a name="deletelinks"></a>Excluindo links de relações
 ...
 Se a função tiver uma multiplicidade de 0.. 1 ou 1.. 1, você poderá defini- `null`la como, ou para outro valor:

 `edward.FamilyTreeModel = null;`or

 `edward.FamilyTreeModel = anotherFamilyTree;`

## <a name="reorder"></a>Reordenando os links de uma relação
...
|Diagrama|<xref:Microsoft.VisualStudio.Modeling.Diagrams.Diagram>|
 ...
 Um elemento em um diagrama geralmente representa um elemento de modelo. Normalmente (mas nem sempre), um <xref:Microsoft.VisualStudio.Modeling.Diagrams.NodeShape> representa uma instância de classe de domínio e <xref:Microsoft.VisualStudio.Modeling.Diagrams.BinaryLinkShape> um representa uma instância de relação de domínio. A <xref:Microsoft.VisualStudio.Modeling.Diagrams.PresentationViewsSubject> relação vincula um nó ou forma de link ao elemento de modelo que ele representa.

 Cada nó ou forma de link pertence a um diagrama. Uma forma de link binário conecta duas formas de nó.

 As formas podem ter formas filhas em dois conjuntos. Uma forma no `NestedChildShapes` conjunto é confinada à caixa delimitadora de seu pai. Uma forma na `RelativeChildShapes` lista pode aparecer fora ou parcialmente fora dos limites do pai – por exemplo, um rótulo ou uma porta. Um diagrama não tem `RelativeChildShapes` nenhum `Parent`.

### <a name="views"></a>Navegando entre formas e elementos
 Elementos de modelo de domínio e elementos de <xref:Microsoft.VisualStudio.Modeling.Diagrams.PresentationViewsSubject> forma são relacionados pela relação.
...
### <a name="merge"></a>Criando um elemento e sua forma
...
### <a name="use-transactions"></a>Usar transações
 Formas, conectores e diagramas são subtipos de <xref:Microsoft.VisualStudio.Modeling.ModelElement> e ao vivo na loja. Portanto, você deve fazer alterações a eles somente dentro de uma transação. Para obter mais informações, confira [Como: Use transações para atualizar o modelo](../modeling/how-to-use-transactions-to-update-the-model.md).

## <a name="docdata"></a>Exibição de documentos e dados de documentos
...
## <a name="see-also"></a>Consulte também
 <xref:Microsoft.VisualStudio.Modeling.ModelElement>[Validação em uma linguagem específica de domínio](../modeling/validation-in-a-domain-specific-language.md) [Gerando código de uma linguagem específica de domínio](../modeling/generating-code-from-a-domain-specific-language.md) [Como: Usar transações para atualizar os modelos](../modeling/how-to-use-transactions-to-update-the-model.md) de integração de modelo [usando o Visual Studio ModelBus](../modeling/integrating-models-by-using-visual-studio-modelbus.md) [responder e propagar alterações](../modeling/responding-to-and-propagating-changes.md)
