...
- Embora o idioma no qual os fragmentos de código são gravados seja C#, você pode gerar texto de qualquer tipo. Como alternativa, você pode escrever o código no [!INCLUDE[vbprvb](../includes/vbprvb-md.md)] adicionando a propriedade `language="VB"` à `template` diretiva.

- Para depurar o modelo, adicione `debug="true"` à `template` diretiva. O modelo será aberto em outra instância do [!INCLUDE[vsprvs](../includes/vsprvs-md.md)] se ocorrer uma exceção. Se você quiser dividir o depurador em um ponto específico no código, insira a instrução `System.Diagnostics.Debugger.Break();`
