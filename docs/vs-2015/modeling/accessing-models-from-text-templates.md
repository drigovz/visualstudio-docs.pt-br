...
- Especifique os processadores de diretiva para a DSL que você deseja acessar. Isso carrega os assemblies para a sua DSL para que você possa usar suas classes de domínio, propriedades e relações no código do seu modelo de texto. Ele também carrega o arquivo de modelo que você especificar.
  ...
  Um `.tt` arquivo semelhante ao exemplo a seguir é criado no projeto de depuração quando você cria uma nova [!INCLUDE[vsprvs](../includes/vsprvs-md.md)] solução a partir do modelo de linguagem mínima de DSL.
...
- Embora o idioma no qual os fragmentos de código são gravados seja C#, você pode gerar texto de qualquer tipo. Como alternativa, você pode escrever o código [!INCLUDE[vbprvb](../includes/vbprvb-md.md)] no adicionando a propriedade `language="VB"` à `template` diretiva.

- Para depurar o modelo, adicione `debug="true"` `template` à diretiva. O modelo será aberto em outra instância do [!INCLUDE[vsprvs](../includes/vsprvs-md.md)] se ocorrer uma exceção. Se você quiser dividir o depurador em um ponto específico no código, insira a instrução`System.Diagnostics.Debugger.Break();`
...
1. Os `filename` parâmetros `validation` e são separados por ";" e não deve haver outros separadores ou espaços.
...
## <a name="loading-models-dynamically"></a>Carregando modelos dinamicamente
 ...
 Em um cenário menos comum, talvez você queira abrir um arquivo de modelo para o qual você tem apenas um nome de arquivo e que pode não estar no [!INCLUDE[vsprvs](../includes/vsprvs-md.md)] projeto atual. Nesse caso, você pode abrir o arquivo usando a técnica descrita em [como: Abra um modelo do arquivo no código](../modeling/how-to-open-a-model-from-file-in-program-code.md)do programa.
...
```
 ...
 `LoopSplitter.tt``LoopTemplate.t4`invoca e, em seguida, divide o arquivo resultante em seus segmentos. Observe que esse modelo não precisa ser um modelo de modelagem, pois ele não lê o modelo.
