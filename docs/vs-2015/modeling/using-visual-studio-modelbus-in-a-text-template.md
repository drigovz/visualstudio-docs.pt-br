...
## <a name="creating-a-model-bus-adapter-for-access-from-text-templates"></a>Criando um adaptador de barramento de modelo para acessar de modelos de texto
 Para resolver uma referência de ModelBus em um modelo de texto, a DSL de destino deve ter um adaptador compatível. Os modelos de texto são executados em um AppDomain separado dos editores de documento [!INCLUDE[vsprvs](../includes/vsprvs-md.md)] e, portanto, o adaptador precisa carregar o modelo em vez de acessá-lo por meio do DTE.

#### <a name="to-create-a-modelbus-adapter-that-is-compatible-with-text-templates"></a>Para criar um adaptador ModelBus compatível com modelos de texto

1. Se a solução de DSL de destino não tiver um projeto **ModelBusAdapter** , crie um usando o assistente de extensão ModelBus:

    1. Baixe e instale a extensão [!INCLUDE[vsprvs](../includes/vsprvs-md.md)] ModelBus, caso ainda não tenha feito isso. Para obter mais informações, consulte [visualização e SDK de modelagem](http://go.microsoft.com/fwlink/?LinkID=185579).
...
## <a name="writing-a-text-template-that-can-resolve-modelbus-references"></a>Escrevendo um modelo de texto que pode resolver referências ModelBus
...
```
 ...
 Quando esse modelo de texto é executado, a diretiva `SourceDsl` carrega o arquivo `Sample.source`. O modelo pode acessar os elementos desse modelo, começando em `this.ModelRoot`. O código pode usar as classes de domínio e as propriedades dessa DSL.
...
2. No diagrama de definição de DSL, clique com o botão direito do mouse em uma parte em branco do diagrama que não está próximo da parte superior e, em seguida, clique em **habilitar ModelBus**.
   ...
   - Se você não vir **habilitar ModelBus**, será necessário baixar e instalar a extensão VMSDK ModelBus. Encontre-o no site do VMSDK: [SDK de visualização e modelagem](http://go.microsoft.com/fwlink/?LinkID=185579).
...
#### <a name="to-create-a-modelbus-adapter-that-is-configured-for-text-templates"></a>Para criar um adaptador ModelBus configurado para modelos de texto
...
2. No projeto DSL, adicione uma referência ao assembly de DSL MBProvider. Clique com o botão direito do mouse em `MBConsumer\Dsl\References` e clique em **Adicionar referência**. Na guia **procurar** , localize `MBProvider\Dsl\bin\Debug\Company.MBProvider.Dsl.dll`
...
5. Salve o arquivo.
   ...
   Você criou um modelo que contém uma referência de ModelBus para um elemento em outro modelo.
...
1. Na instância experimental do [!INCLUDE[vsprvs](../includes/vsprvs-md.md)], abra um arquivo de modelo de texto de exemplo. Defina seu conteúdo da seguinte maneira.
    ...
    2. O modelo de consumidor é acessado da maneira usual por meio do processador de diretiva gerado nesse DSL.

    3. As diretivas assembly e Import devem ser capazes de acessar ModelBus e os tipos de DSL do provedor.

    4. Se você souber que muitos MBRs estão vinculados ao mesmo modelo, é melhor chamar createadapter apenas uma vez.
