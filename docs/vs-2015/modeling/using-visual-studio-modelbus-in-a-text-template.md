...
1. Se a solução DSL de destino não possui um **ModelBusAdapter** de projeto, crie uma usando o Assistente de extensão do Modelbus:
    ...
    1. Baixe e instale a extensão [!INCLUDE[vsprvs](../includes/vsprvs-md.md)] ModelBus, caso ainda não tenha feito isso. Para obter mais informações, consulte [SDK de visualização e modelagem](https://go.microsoft.com/fwlink/?LinkID=185579).
...
2. No diagrama de definição de DSL, uma parte em branco do diagrama que não está na parte superior com o botão direito e, em seguida, clique em **habilitar Modelbus**.
   ...
   - Se você não vir **habilitar Modelbus**, você deve baixar e instalar a extensão do ModelBus VMSDK. Encontrá-lo no site do VMSDK: [SDK de visualização e modelagem](https://go.microsoft.com/fwlink/?LinkID=185579).
