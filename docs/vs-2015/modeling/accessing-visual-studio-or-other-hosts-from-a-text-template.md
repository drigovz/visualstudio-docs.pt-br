...
Defina `hostspecific="true"` na `template` diretiva. Isso permite que você use  `this.Host` , que tem o tipo [ITextTemplatingEngineHost](/previous-versions/visualstudio/visual-studio-2012/bb126505(v=vs.110)). Esse tipo tem membros que você pode usar, por exemplo, para resolver nomes de arquivo e registrar erros.
...
## <a name="using-the-visual-studio-api"></a>Usando a API do Visual Studio
 Se você estiver executando um modelo de texto no [!INCLUDE[vsprvs](../includes/vsprvs-md.md)] , poderá usar o `this.Host` para acessar os serviços fornecidos pelo [!INCLUDE[vsprvs](../includes/vsprvs-md.md)] e quaisquer pacotes ou extensões que estejam carregados.
...
## <a name="using-hostspecific-with-template-inheritance"></a>Usando hostSpecific com herança de modelo
 Especifique `hostspecific="trueFromBase"` se você também usa o `inherits` atributo e se herda de um modelo que especifica `hostspecific="true"` . Isso evita um aviso do compilador para o efeito de que a propriedade foi `Host` declarada duas vezes.
