---
title: Noções básicas sobre o código DSL | Microsoft Docs
ms.date: 11/15/2016
...
> [!NOTE]
> O modelo DSL de diagramas de componentes não está relacionado aos diagramas de componentes UML que você pode criar usando o menu arquitetura no Visual Studio. Na caixa de diálogo **novo projeto** , expanda **outro projeto Types\Extensibility** e clique em **Designer de linguagem específica de domínio**.
...
- Grave seus próprios modelos de texto para gerar código específico para o aplicativo. É possível usar arquivos de inclusão para compartilhar partes dos modelos que são comuns a diversos projetos e, é possível criar modelos de projeto do [!INCLUDE[vsprvs](../includes/vsprvs-md.md)] para definir projetos que são inicializados com a própria estrutura de arquivos.
...
> Não é a mesma que a classe raiz do modelo.
 ...
 Esse arquivo é associado ao assembly de compilação e fornece os valores padrão desses recursos. É possível localizar a DSL criando um assembly satélite que contém uma versão localizada dos recursos. Essa versão será usada quando a DSL for instalada em uma cultura que corresponde aos recursos localizados. Para obter mais informações, confira [Implantando soluções de linguagem específica de domínio](../modeling/deploying-domain-specific-language-solutions.md).
...
- Classe SerializationHelper, que fornece funções que são usadas em comum pelas classes de serialização.
  ...
  Para substituir os métodos gerados pelos seus próprios métodos em uma classe parcial, Set **gera Double Derived** para o conector na definição de DSL. Para substituir um construtor por seu próprio código, Set **tem um construtor personalizado**.
...
```
 ...
 Instancia `DocData` e `DocView`. Atende a uma interface padrão que o [!INCLUDE[vsprvs](../includes/vsprvs-md.md)] usa para abrir um editor quando o pacote DSL é iniciado. É referenciado no atributo `ProvideEditorFactory` em Package.cs

 `GeneratedVSCT.vsct`

 Localiza os comandos de menu padrão em menus, como o menu de contexto do diagrama, o menu **Editar** e assim por diante. O código dos comandos está em CommandSet.cs. É possível realocar ou modificar os comandos padrão e adicionar seus próprios comandos. Para obter mais informações, consulte [Writing User Commands and actions](../modeling/writing-user-commands-and-actions.md).
...
```
 ...
 Esse arquivo define como a DSL integra no [!INCLUDE[vsprvs](../includes/vsprvs-md.md)]. Atributos na classe do pacote registram a DSL como o manipulador de arquivos que possuem sua extensão de arquivo, define a caixa de ferramentas e define como abrir uma nova janela. O método Inicializar() é chamado uma vez quando a primeira DSL é chamada para uma instância do [!INCLUDE[vsprvs](../includes/vsprvs-md.md)].
