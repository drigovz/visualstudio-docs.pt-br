...
- O nome de um arquivo incluído não precisa usar a extensão `".tt"`.
   ...
   Você pode usar outra extensão como `".t4"` para arquivos incluídos. Isso ocorre porque, quando você adiciona um arquivo de `.tt` a um projeto, [!INCLUDE[vsprvs](../includes/vsprvs-md.md)] define automaticamente sua propriedade de **ferramenta personalizada** como `TextTemplatingFileGenerator`. Geralmente, você não quer que os arquivos incluídos sejam transformados individualmente.
