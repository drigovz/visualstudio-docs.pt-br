...
|--------------|-----------------|-----------------------------|  
|`MSBuildBinPath`|O caminho absoluto da pasta em que os [!INCLUDE[vstecmsbuild](../includes/vstecmsbuild-md.md)] binários que estão sendo usados no momento estão localizados (por exemplo, C:\Windows\Microsoft.NET\Framework \\ *versionNumber*). Essa propriedade será útil se você precisar fazer referência a arquivos no diretório [!INCLUDE[vstecmsbuild](../includes/vstecmsbuild-md.md)].<br /><br /> Não inclua a barra invertida final nessa propriedade.|Reservado|  
