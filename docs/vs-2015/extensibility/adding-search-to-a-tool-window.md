...
6. Na barra de menus, escolha **Exibir**, **outras janelas**, **TestSearch**.  
...
1. No arquivo TestSearch.cs, adicione as seguintes `using` instruções:  
...
5. Teste as novas configurações recriando a solução e iniciando para debugb.  
...
2. Na `TestSearchTask` classe, remova a marca de comentário da linha MatchCase no `OnStartSearch` método:  
