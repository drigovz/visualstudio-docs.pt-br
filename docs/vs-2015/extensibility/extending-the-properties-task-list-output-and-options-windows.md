---
title: Estendendo as propriedades, Lista de Tarefas, saída e opções Windows | Microsoft Docs
ms.date: 11/15/2016
...
1. Adicione uma classe chamada `ToolsOptions.cs`. Faça com que a classe ToolsOptions seja herdada <xref:Microsoft.VisualStudio.Shell.DialogPage> .  
...
2. O primeiro parâmetro para o Construtor ProvideOptionPage é o tipo de ferramentas de classe, que você criou anteriormente. O segundo parâmetro, "ToDo", é o nome da categoria na caixa de diálogo **Opções** . O terceiro parâmetro, "General", é o nome da subcategoria da caixa de diálogo **Opções** em que a página opções estará disponível. Os dois parâmetros seguintes são IDs de recurso para cadeias de caracteres; o primeiro é o nome da categoria e o segundo é o nome da subcategoria. O parâmetro final determina se essa página pode ser acessada usando a automação.  
...
## <a name="make-data-available-to-the-properties-window"></a>Tornar os dados disponíveis para a janela Propriedades  
 Você pode fazer as informações da lista de tarefas disponíveis criando uma classe chamada TodoItem que armazena informações sobre os itens individuais na lista de tarefas.  
...
2. Adicione o seguinte usando instruções do arquivo TodoItem.cs.  
...
6. No TodoWindowControl.xaml.cs, adicione métodos stub à classe TodoWindowControl para os `CheckForError` métodos e `UpdateList` . Coloque-os após ProcessDialogChar e antes do final do arquivo.  
...
## <a name="integrate-into-the-properties-window"></a>Integrar na janela Propriedades  
...
1. Substitua a `button1_Click` função existente pelo código que cria um novo TodoItem e o adiciona à caixa de listagem. Ele chama TrackSelection (), que será definido posteriormente.  
...
9. Abra as páginas **ferramentas/opções** . Você deve ver a categoria ToDo no painel esquerdo. As categorias são listadas em ordem alfabética, portanto, olhe sob o TS.  
...
## <a name="add-text-to-the-output-window-and-items-to-the-task-list"></a>Adicionar texto à Janela de Saída e aos itens ao Lista de Tarefas  
 Para o **lista de tarefas**, você cria um novo objeto do tipo Task e, em seguida, adiciona esse objeto de tarefa ao **lista de tarefas** chamando seu método Add. Para gravar na janela de **saída** , chame o método GetPane para obter um objeto de painel e, em seguida, chame o método OutputString do objeto de painel.  
...
4. Adicione ClearError (), que limpa o Lista de Tarefas e ReportError (), que adiciona uma entrada ao Lista de Tarefas à classe TodoWindowControl.  
...
4. Agora, altere a configuração na página **ferramentas/opções/todo** de **2** para **0**.  
