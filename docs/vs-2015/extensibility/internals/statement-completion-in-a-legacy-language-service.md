...
## <a name="implementing-statement-completion"></a>Implementando a conclusão da instrução  
 ...
 Para implementar a conclusão da instrução, seu idioma deve ter um gatilho de conclusão de instrução, que pode ser analisado. Por exemplo, [!INCLUDE[vbprvb](../../includes/vbprvb-md.md)] usa um operador de ponto (.), enquanto [!INCLUDE[vcprvc](../../includes/vcprvc-md.md)] usa um operador de seta (->). Um serviço de linguagem pode usar mais de um gatilho para iniciar a conclusão da instrução. Esses gatilhos são programados no filtro de comando.  
