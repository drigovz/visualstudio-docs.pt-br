...
## <a name="parameters"></a>Parâmetros  
 pvCallerData  
 O `pvCallerData` parâmetro passado pelo chamador (o IDE) para o [SccPopulateList](../extensibility/sccpopulatelist-function.md). O plug-in de controle do código-fonte deve assumir nada sobre o conteúdo desse parâmetro.  
 ...
 nStatus  
 Status de `lpFileName` (uma combinação de `SCC_STATUS` bits; consulte o [código de status do arquivo](../extensibility/file-status-code-enumerator.md) para obter detalhes).  
  
 lpFileName  
 Caminho completo do diretório do nome do arquivo a ser adicionado ou excluído da lista.  
