...
|`OpenInEditor`|Atributo opcional.<br /><br /> Um valor booliano que especifica se o item deve ser aberto em seu respectivo editor em [!INCLUDE[vsprvs](../includes/vsprvs-md.md)] quando um projeto é criado a partir do modelo.<br /><br /> Os `OpenInWebBrowser` `OpenInHelpBrowser` atributos e são ignorados em um item com um `OpenInEditor` valor de `true` .<br /><br /> O valor padrão é `false`.|  
