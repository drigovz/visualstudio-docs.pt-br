...
- [Insira o arquivo de manifesto no arquivo executável da solução publicada](#BKMK_embedmanifest).

- [Assine os manifestos de aplicativo e implantação](#BKMK_signappdeplyman).
...
## <a name="publish-the-solution-without-visual-styles-enabled"></a><a name="BKMK_publishsolwovs"></a> Publicar a solução sem estilos visuais habilitados
...
1. Verifique se o projeto não tem estilos visuais habilitados. Primeiro, verifique o arquivo de manifesto do seu projeto para o seguinte XML. Em seguida, se o XML estiver presente, coloque o XML com uma marca de comentário.
    ...
    **Para abrir o arquivo de manifesto em um projeto Visual Basic**
    ...
    1. Na barra de menus, escolha **Project**, _ProjectName_**Propriedades**ProjectName, em que *ProjectName* é o nome do seu projeto do WPF.
...
## <a name="create-a-manifest-file"></a><a name="BKMK_CreateManifest"></a> Criar um arquivo de manifesto
...
## <a name="embed-the-manifest-file-into-the-executable-file-of-the-published-solution"></a><a name="BKMK_embedmanifest"></a> Inserir o arquivo de manifesto no arquivo executável da solução publicada
...
## <a name="sign-the-application-and-deployment-manifests"></a><a name="BKMK_signappdeplyman"></a> Assinar os manifestos de aplicativo e implantação

1. No prompt de comando, execute o seguinte comando para remover a `.deploy` extensão do arquivo executável no diretório atual.
   ...
   > [!NOTE]
   > Este exemplo pressupõe que apenas um arquivo tem a `.deploy` extensão de arquivo. Certifique-se de renomear todos os arquivos neste diretório que têm a `.deploy` extensão de arquivo.
...
2. No prompt de comando, execute o comando a seguir para assinar o manifesto do aplicativo.
   ...
   > [!NOTE]
   > Este exemplo pressupõe que você assine o manifesto usando o `.pfx` arquivo do projeto. Se você não estiver assinando o manifesto, poderá omitir o `–cf` parâmetro usado neste exemplo. Se você estiver assinando o manifesto com um certificado que requer uma senha, especifique a `–password` opção ( `For example: mage –u MyWPFApp.exe.manifest –cf ..\..\..\MyWPFApp_TemporaryKey.pfx – password Password` ).

3. No prompt de comando, execute o seguinte comando para adicionar a `.deploy` extensão ao nome do arquivo que você renomeou em uma etapa anterior deste procedimento.
   ...
   > [!NOTE]
   > Este exemplo pressupõe que apenas um arquivo tinha uma `.deploy` extensão de arquivo. Certifique-se de renomear todos os arquivos nesse diretório que anteriormente tinham a `.deploy` extensão de nome de arquivo.
...
[Solucionando problemas de erros específicos em implantações](../deployment/troubleshooting-specific-errors-in-clickonce-deployments.md) 
 do ClickOnce [Visão geral dos](https://msdn.microsoft.com/5b5d7bb6-684f-478d-bf5f-b8d18bbcff2e) 
 estilos visuais [Prompts de comando](https://msdn.microsoft.com/library/94fcf524-9045-4993-bfb2-e2d8bad44219)
