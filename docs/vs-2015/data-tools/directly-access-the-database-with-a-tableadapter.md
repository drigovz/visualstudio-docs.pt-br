...
Além dos `InsertCommand` `UpdateCommand` `DeleteCommand` TableAdapters, e, são criados com métodos que podem ser executados diretamente no banco de dados. Esses métodos ( `TableAdapter.Insert` , `TableAdapter.Update` e `TableAdapter.Delete` ) podem ser chamados para manipular dados diretamente no banco de dado.
