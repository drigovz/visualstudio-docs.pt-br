...
Atualização hierárquica * refere-se ao processo de salvar dados atualizados (a partir de um DataSet com duas ou mais tabelas relacionadas) de volta a um banco de dado, mantendo regras de integridade referencial. A *integridade referencial* refere-se às regras de consistência fornecidas pelas restrições em um banco de dados que controlam o comportamento de inserção, atualização e exclusão de registros relacionados. Por exemplo, é a integridade referencial que impõe a criação de um registro de cliente antes de permitir que os pedidos sejam criados para esse cliente.  Para obter mais informações sobre relações em conjuntos de dados, consulte [relações em conjuntos de](../data-tools/relationships-in-datasets.md) dados
 ...
 O recurso de atualização hierárquica usa um `TableAdapterManager` para gerenciar o `TableAdapter`s em um dataset tipado. O componente `TableAdapterManager` é uma classe gerada por [!INCLUDE[vsprvs](../includes/vsprvs-md.md)], portanto, não faz parte da [!INCLUDE[dnprdnshort](../includes/dnprdnshort-md.md)]. Quando você arrasta uma tabela da janela fontes de dados para uma página do Windows Form ou WPF, o Visual Studio adiciona uma variável do tipo TableAdaptermanager ao formulário ou à página e você a vê no designer na bandeja de componentes. Para obter informações detalhadas sobre a classe `TableAdapterManager`, consulte a seção referência do TableAdaptermanager da [visão geral do TableAdapterManager](https://msdn.microsoft.com/library/33076d42-6b41-491a-ac11-6c6339aea650).
...
> É importante entender que a ordem de atualização é tudo inclusiva. Ou seja, quando as atualizações são executadas, as inserções e as exclusões são executadas para todas as tabelas no conjunto de conjuntos.
 ...
 Para definir a propriedade `UpdateOrder`, depois de arrastar itens da [janela fontes de dados](https://msdn.microsoft.com/library/0d20f699-cc95-45b3-8ecb-c7edf1f67992) para um formulário, selecione o `TableAdapterManager` na bandeja de componentes e, em seguida, defina a propriedade `UpdateOrder` na janela **Propriedades** . Para obter mais informações, consulte [como: definir a ordem ao executar uma atualização hierárquica](https://msdn.microsoft.com/library/a0734935-78dd-4c0b-80d7-5e7925789c83).
...
## <a name="tableadaptermanager-reference"></a>Referência do TableAdaptermanager
 ...
 A classe `TableAdapterManager` não faz parte da [!INCLUDE[dnprdnshort](../includes/dnprdnshort-md.md)]. Portanto, você não pode procurar na documentação. Ele é criado no momento do design como parte do processo de criação do conjunto de um.
