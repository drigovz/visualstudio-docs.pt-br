...
- Reduzir informações não acionáveis nos dados heap JavaScript.  
  ...
  [Executar o analisador de memória JavaScript](#Run)   
  [Verificar o uso de memória](#Check)   
  [Isolar um vazamento de memória](#Isolate)   
  [Exibir Resumo de uso de memória dinâmica](#LiveMemory)   
  [Exibir um resumo de instantâneo](#SnapshotSummary)   
  [Exibir detalhes do instantâneo](#SnapshotDetails)   
  [Exibir uma comparação de instantâneo](#SnapshotDiff)   
  ...
  [Exibir referências de objeto compartilhado](#References)   
  [Mostrar objetos internos](#BuiltInValues)   
  [Salvar arquivos de sessão de diagnóstico](#Save)   
  [Associar o código-fonte a dados de uso de memória](#JSConsoleCommands)   
  [Dicas para identificar problemas de memória](#Tips)  
  
## <a name="run-the-javascript-memory-analyzer"></a><a name="Run"></a> Executar o analisador de memória JavaScript  
...
## <a name="check-memory-usage"></a><a name="Check"></a> Verificar o uso de memória  
...
- [Exiba um resumo de instantâneo](#SnapshotSummary). Você pode exibir informações resumidas de instantâneo durante ou após uma sessão de criação de perfil de memória. Use os resumos de instantâneo para acessar detalhes de instantâneo e exibições diferenciais de instantâneo.  
...
- [Exibir uma comparação de instantâneos](#SnapshotDiff). Mostra valores diferenciais entre instantâneos. Essas exibições mostram diferenças de tamanho e número de objetos.  
  
## <a name="isolate-a-memory-leak"></a><a name="Isolate"></a> Isolar uma perda de memória  
...
10. Alterne para o Visual Studio e obtenha um terceiro instantâneo.  
     ...
     ![Segundo e terceiro instantâneo](../profiling/media/js-mem-leak-workflow.png "JS_Mem_Leak_Workflow")  
...
12. Escolha uma das seguintes opções de filtro de **Escopo** para ajudar a identificar problemas de uso de memória:  
    ...
    > Use a exibição filtrada de objetos restantes desde o instantâneo anterior para investigar os vazamentos de memória. Por exemplo, se a contagem de objetos diferencial for de +205 / -195, essa exibição mostrará os 10 objetos restantes, os quais serão as prováveis explicações para os vazamentos de memória.  
     ...
     ![Exibição de comparação de instantâneos mostrando tipos](../profiling/media/js-mem-snapshot-diff.png "JS_Mem_Snapshot_Diff")  
...
## <a name="view-live-memory-usage-summary"></a><a name="LiveMemory"></a> Exibir Resumo de uso de memória dinâmica  
 ...
 ![Grafo de memória do analisador de memória JavaScript](../profiling/media/js-mem-memory-graph.png "JS_Mem_Memory_Graph")  
...
## <a name="view-a-snapshot-summary"></a><a name="SnapshotSummary"></a> Exibir um resumo de instantâneo  
...
> O analisador de memória de JavaScript força uma coleta de lixo antes de cada instantâneo. Isso ajuda a garantir mais resultados consistentes entre as execuções.  
 ...
 ![Resumo do instantâneo](../profiling/media/js-mem-snapshot-summary.png "JS_Mem_Snapshot_Summary")  
...
## <a name="view-snapshot-details"></a><a name="SnapshotDetails"></a> Exibir detalhes do instantâneo  
 ...
 ![Exibição de detalhes do instantâneo mostrando possíveis problemas](../profiling/media/js-mem-snapshot-details.png "JS_Mem_Snapshot_Details")  
...
## <a name="view-a-snapshot-diff"></a><a name="SnapshotDiff"></a> Exibir comparações de um instantâneo  
...
## <a name="view-objects-by-dominator"></a><a name="FoldObjects"></a> Exibir objetos por dominador  
 Nas exibições Tipos e Dominadores, você pode escolher ver os objetos classificados pelos dominadores (esta é a exibição padrão da guia Dominadores). Ao selecionar esta exibição, somente os dominadores são mostrados na exibição superior dos objetos. (Objetos que são descendentes de objetos não globais são ocultados da exibição de nível superior.) Para alguns aplicativos, isso pode esclarecer quais objetos estão causando um vazamento de memória, reduzindo o ruído nos dados.  
...
## <a name="filter-data-by-identifier"></a><a name="Filter"></a> Filtrar dados por identificador  
...
## <a name="find-an-object-in-the-object-tree"></a><a name="ShowInRootsView"></a> Localizar um objeto na árvore de objetos  
...
## <a name="view-shared-object-references"></a><a name="References"></a> Exibir referências de objeto compartilhado  
...
> Referências circulares são exibidas com um (*) asterisco e uma dica de ferramenta informativa e não podem ser expandidas. Do contrário, elas impediriam que você percorra a árvore de referências e identifique objetos que estão retendo memória.  
 ...
 Se você quiser obter ajuda adicional para identificar objetos equivalentes, escolha **Exibir IDs de objeto** nas configurações listar ![configurações drop&#45;lista suspensa no analisador de memória](../profiling/media/js-mem-settings.png "JS_Mem_Settings") no canto superior direito do painel superior. Essa opção exibe IDs de objetos ao lado dos nomes dos objetos na lista **Identificador(es)** (as IDs aparecem em todas as exibições, não apenas na lista de Referências de objeto). Os objetos que têm a mesma ID são referências compartilhadas.  
  
 A ilustração a seguir mostra a lista Referências de objeto para um item selecionado com as IDs exibidas.  
  
 ![Referências de objeto com IDs exibidas](../profiling/media/js-mem-shared-refs.png "JS_Mem_Shared_Refs")  
  
## <a name="show-built-in-objects"></a><a name="BuiltInValues"></a> Mostrar objetos internos  
 ...
 Para exibir esses objetos, escolha **Mostrar incorporados** na lista configurações&#45;lista ![de configurações, menu para baixo no analisador de memória](../profiling/media/js-mem-settings.png "JS_Mem_Settings") no canto superior direito do painel.  
  
## <a name="save-diagnostic-session-files"></a><a name="Save"></a> Salvar arquivos de sessão de diagnóstico  
...
## <a name="associate-source-code-with-memory-usage-data"></a><a name="JSConsoleCommands"></a> Associar o código-fonte a dados de uso de memória  
...
## <a name="tips-for-identifying-memory-issues"></a><a name="Tips"></a> Dicas para identificar problemas de memória  
  
- Siga o fluxo de trabalho descrito em [isolar uma perda de memória](#Isolate) e use os **objetos restantes \<number> do filtro instantâneo #** em uma exibição de comparação para identificar prováveis candidatos a vazamentos de memória.  
...
## <a name="see-also"></a>Consulte Também  
 [Instruções passo a passo: localizar uma perda de memória (JavaScript)](../profiling/walkthrough-find-a-memory-leak-javascript.md)
