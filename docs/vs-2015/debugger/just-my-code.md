...
ms.locfileid: "78410155"
---
# <a name="just-my-code"></a>Apenas Meu Código
[!INCLUDE[vs2017banner](../includes/vs2017banner.md)]

Os desenvolvedores que usam as linguagens do .NET Framework estão familiarizados com o depurador Apenas Meu Código que considera chamadas de sistema, de estrutura e outras chamadas não usuário e recolhe essas chamadas nas janelas de pilha de chamadas. Apenas Meu Código foi estendido para as C++ linguagens do e do JavaScript. Este tópico descreve detalhes específicos do uso do Apenas Meu Código em projetos do .NET Framework, nativos do C++ e de JavaScript.  
  
## <a name="BKMK_Enable_or_disable_Just_My_Code"></a> Habilitar ou desabilitar Apenas Meu Código  
 Para habilitar ou desabilitar Apenas Meu Código, escolha **Opções e configurações** no menu **depurar** . No nó / **geral** de **depuração** , escolha ou desmarque **habilitar apenas meu código**.  
...
### <a name="BKMK_Override_call_stack_filtering"></a>Substituir filtragem de pilha de chamadas  
 Em exibição da pilha de chamadas, como as janelas pilha de chamadas e tarefas, Apenas Meu Código recolhe o código que não é do usuário em um quadro anotado, rotulado `[External Code]`. Para exibir os quadros recolhidos, escolha **Mostrar código externo** no menu de contexto da exibição da pilha de chamadas.  
...
## <a name="BKMK__NET_Framework_Just_My_Code"></a>.NET Framework Apenas Meu Código  
  
### <a name="BKMK_NET_User_and_non_user_code"></a>Usuário e código de não usuário  
...
- <xref:System.Diagnostics.DebuggerNonUserCodeAttribute> informa ao depurador que o código ao qual ele é aplicado não é meu código.  
...
### <a name="BKMK_NET_Breakpoint_behavior"></a>Comportamento do ponto de interrupção  
...
### <a name="BKMK_NET_Exception_behavior"></a>Comportamento da exceção  
...
### <a name="BKMK_CPP_User_and_non_user_code"></a>Usuário e código de não usuário  
...
- Funções especificadas em `*.natjmc` arquivos na pasta `%VsInstallDirectory%\Common7\Packages\Debugger\Visualizers`.  
  
  **Depurar**  
  
  Por padrão, somente as funções especificadas em `*.natstepfilter` arquivos na pasta `%VsInstallDirectory%\Common7\Packages\Debugger\Visualizers` são consideradas código de não usuário.  
  
  Você pode criar seu próprio `.natstepfilter` e `.natjmc` para personalizar o comportamento da janela de etapa e pilha de chamadas no `%USERPROFILE%\My Documents\Visual Studio 2015\Visualizers`.  
...
### <a name="BKMK_CPP_Exception_behavior"></a>Comportamento da exceção  
...
### <a name="BKMK_CPP_Customize_stepping_behavior"></a>Personalizar comportamento de depuração  
 Você pode especificar as funções a serem percorrendo, listando-as como código de não usuário em `*.natstepfilter` arquivos.  
  
- Para especificar o código de não-usuário para todos os usuários do computador do Visual Studio, adicione o arquivo. natstepfilter à pasta `%VsInstallDirectory%\Common7\Packages\Debugger\Visualizers`.  
  
- Para especificar um código que não seja de usuário para um usuário individual, adicione o arquivo. natstepfilter à pasta `%USERPROFILE%\My Documents\Visual Studio 2015\Visualizers`.  
...
### <a name="BKMK_CPP_Customize_call_stack_behavior"></a>Personalizar o comportamento da pilha de chamadas  
 Você pode especificar módulos, arquivos de origem e funções para tratar como código não-usuário em pilhas de chamadas, especificando-os em arquivos `*.natjmc`.  
  
- Para especificar o código de não-usuário para todos os usuários do computador do Visual Studio, adicione o arquivo. natjmc à pasta `%VsInstallDirectory%\Common7\Packages\Debugger\Visualizers`.  
  
- Para especificar um código que não seja de usuário para um usuário individual, adicione o arquivo. natjmc à pasta `%USERPROFILE%\My Documents\Visual Studio 2015\Visualizers`.  
...
|Atributo|DESCRIÇÃO|  
|---------------|-----------------|  
|`Name`|Obrigatórios. O caminho completo do módulo ou dos módulos. Você pode usar os caracteres curinga do Windows `?` (zero ou um caractere) e `*` (zero ou mais caracteres). Por exemplo,<br /><br /> `<Module Name=”?:\3rdParty\UtilLibs\*” />`<br /><br /> instrui o depurador a tratar todos os módulos em `\3rdParty\UtilLibs` em qualquer unidade como código externo.|  
...
### <a name="BKMK_JS_User_and_non_user_code"></a>Usuário e código de não usuário  
...
- O script que é executado passando uma cadeia de caracteres para a função de `eval` fornecida pelo host é classificado como **MyCode**.  
  
- O script que é executado passando uma cadeia de caracteres para o construtor de `Function` é classificado como **LibraryCode**.  
  
- O script contido em uma referência de estrutura, como WinJS ou SDK do Azure, é classificado como **LibraryCode**.  
  
- O script que é executado passando uma cadeia de caracteres para as funções `setTimeout`, `setImmediate`ou `setInterval` é classificado como **UnrelatedCode**.  
...
### <a name="BKMK_JS_Breakpoint_behavior"></a>Comportamento do ponto de interrupção  
...
### <a name="BKMK_JS_Exception_behavior"></a>Comportamento da exceção  
...
2. Classificações no arquivo de `%VSInstallDirectory%\JavaScript\JustMyCode\mycode.default.wwa.json`  
