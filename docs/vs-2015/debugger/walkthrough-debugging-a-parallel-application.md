---
title: 'Walkthrough: Depurando um aplicativo paralelo | Microsoft Docs'
ms.date: 11/15/2016
...
Este passo a passo descreve como usar as janelas **Tarefas Paralelas** e **Pilhas Paralelas** para depurar um aplicativo paralelo. Essas janelas ajudam você a entender e verificar o comportamento do tempo de execução do código que usa a [TPL (biblioteca paralela de tarefas)](https://msdn.microsoft.com/library/b8f99f43-9104-45fd-9bff-385a20488a23) ou a [tempo de execução de simultaneidade](https://msdn.microsoft.com/library/874bc58f-8dce-483e-a3a1-4dcc9e52ed2c). Este passo a passo fornece código de exemplo que tem pontos de interrupção internos. Após a interrupção do código, este passo a passo mostra como usar as janelas **Tarefas Paralelas** e **Pilhas Paralelas** para examiná-lo.  
...
2. No painel **modelos instalados** , selecione Visual C#, Visual Basic ou Visual C++. Para as linguagens gerenciadas, certifique-se de que [!INCLUDE[net_v40_short](../includes/net-v40-short-md.md)] seja exibido na caixa da estrutura.  
...
1. No menu **Depurar**, aponte para **Windows** e, em seguida, clique em **Pilhas Paralelas**. Verifique se **Threads** está selecionado na caixa no canto superior esquerdo.  
     ...
     ![Exibição de threads na janela de pilhas paralelas](../debugger/media/pdb-walkthrough-1.png "PDB_Walkthrough_1")  
     ...
     ![Dica de ferramenta no quadro de ativação ativo](../debugger/media/pdb-walkthrough-1b.png "PDB_Walkthrough_1B")  
     ...
     ![Thread principal realçado na janela pilhas paralelas](../debugger/media/pdb-walkthrough-1c.png "PDB_Walkthrough_1C")  
...
1. Para retomar a execução até que o segundo ponto de interrupção seja atingido, no menu **Depurar**, clique em **Continuar**. A ilustração a seguir mostra a árvore do thread no segundo ponto de interrupção.  
     ...
     ![Caminho da execução de pilhas paralelas](../debugger/media/pdb-walkthrough-2b.png "PDB_Walkthrough_2B")  
     ...
     ![Menu pilhas com duas opções em C enquanto J está atual](../debugger/media/pdb-walkthrough-3.png "PDB_Walkthrough_3")  
     ...
     ![Janela e dica de ferramenta de pilhas paralelas vazias](../debugger/media/pdb-walkthrough-3a.png "PDB_Walkthrough_3A")  
  
     Em seguida, na janela **threads** , sinalize threads um a um para ver como suas pilhas de chamadas aparecem na janela **pilhas paralelas** . Para sinalizar threads, use o menu de atalho ou a primeira célula de um thread. Clique no botão **Mostrar apenas sinalizado** na barra de ferramentas para mostrar todos os threads.  
...
1. Para retomar a execução até que o terceiro ponto de interrupção seja atingido, no menu **Depurar**, clique em **Continuar**.  
    ...
    ![Caminho de execução na janela de pilhas paralelas](../debugger/media/pdb-walkthrough-3b.png "PDB_Walkthrough_3B")  
    ...
    ![Exibição de método na janela pilhas paralelas](../debugger/media/pdw-walkthrough-4.png "PDW_Walkthrough_4")  
   ...
   - As **configurações de símbolo** e informações de **carregamento de símbolo** abrem as respectivas caixas de diálogo.  
  
   - **Vá para código-fonte** e **vá para desmontagem** navegue no editor para o método selecionado.  
   ...
   - **Mostrar Código Externo** exibe todos os registros, mesmo quando eles não estão no código do usuário. Tente verificar o diagrama se expande para acomodar os registros adicionais (que podem ser escurecidos porque você não tem símbolos para eles).  
     ...
     ![Autorolagem na janela pilhas paralelas](../debugger/media/pdb-walkthrough-4a.png "PDB_Walkthrough_4A")  
...
1. Para retomar a execução até que o quarto ponto de interrupção seja atingido, no menu **Depurar**, clique em **Continuar**.  
     ...
     A **Exibição Panorâmica** também é útil com diagramas grandes na janela **Pilhas Paralelas**. Você pode ver o **modo de exibição de visão aérea** clicando no botão entre as barras de rolagem no canto inferior direito da janela, conforme mostrado na ilustração a seguir.  
  
     ![Exibição de olho de&#45;de pássaro na janela de pilhas paralelas](../debugger/media/pdb-walkthrough-5.png "PDB_Walkthrough_5")  
     ...
     ![Pilhas com zoom na janela de pilhas paralelas](../debugger/media/pdb-walkthrough-5a.png "PDB_Walkthrough_5A")  
...
5. No menu **depurar** , aponte para **Windows** e clique em **tarefas paralelas**. A ilustração a seguir mostra a janela **tarefas paralelas** .  
     ...
     ![Quatro tarefas em execução na janela tarefas paralelas](../debugger/media/pdw-walkthrough-6.png "PDW_Walkthrough_6")  
...
1. Para retomar a execução até que o segundo ponto de interrupção seja atingido, no menu **Depurar**, clique em **Continuar**.  
     ...
     Anteriormente, a coluna **status** mostrou todas as tarefas como em execução, mas agora duas das tarefas estão aguardando. As tarefas podem ser bloqueadas por diversos motivos diferentes. Na coluna **Status**, passe o mouse sobre uma tarefa em espera para saber por que ela está bloqueada. Por exemplo, na ilustração, a tarefa 3 está aguardando a tarefa 4.  
     ...
     ![Tarefa de espera e dica de ferramenta na janela tarefas](../debugger/media/pdb-walkthrough-7a.png "PDB_Walkthrough_7A")  
     ...
     ![Exibição de threads na janela de pilhas paralelas](../debugger/media/pdb-walkthrough-8.png "PDB_Walkthrough_8")  
...
1. Para retomar a execução até que o terceiro ponto de interrupção seja atingido, no menu **Depurar**, clique em **Continuar**.  
     ...
     Para visualizar melhor a relação pai-filho, clique com o botão direito do mouse no cabeçalho da coluna **pai** e clique em **exibição pai filho**. Você verá a ilustração a seguir.  
  
     ![Exibição filho de&#45;pai na janela tarefas paralelas](../debugger/media/pdb-walkthrough-9.png "PDB_Walkthrough_9")  
  
     Observe que as tarefas 4 e 5 estão em execução no mesmo thread. Essas informações não são exibidas na janela **threads** ; vê-lo aqui está outro benefício da janela de **tarefas paralelas** . Para confirmar isso, exiba a janela **Pilhas Paralelas**. Verifique se você está exibindo **Tarefas**. Localize as tarefas 4 e 5 clicando duas vezes nelas na janela **tarefas paralelas** . Quando você fizer isso, o realce azul na janela **Pilhas Paralelas** será atualizado. Você também pode localizar as tarefas 4 e 5 examinando as dicas de ferramenta na janela **Pilhas Paralelas**.  
  
     ![Exibição de tarefa na janela de pilhas paralelas](../debugger/media/pdb-walkthrough-9a.png "PDB_Walkthrough_9A")  
     ...
     ![Thread realçado na exibição threads](../debugger/media/pdb-walkthrough-9b.png "PDB_Walkthrough_9B")  
...
1. Para retomar a execução até que o terceiro ponto de interrupção seja atingido, no menu **Depurar**, clique em **Continuar**. Clique no cabeçalho de coluna **ID** para classificar por ID. Você verá a ilustração a seguir.  
     ...
     ![Quatro Estados de tarefa na janela de pilhas paralelas](../debugger/media/pdb-walkthrough-10.png "PDB_Walkthrough_10")  
     ...
     ![Dica de ferramenta de cabeçalho na janela de pilhas paralelas](../debugger/media/pdb-walkthrough-11.png "PDB_Walkthrough_11")  
     ...
     ![Tarefas agrupadas na janela tarefas paralelas](../debugger/media/pdb-walkthrough-12.png "PDB_Walkthrough_12")  
  
     Também é possível agrupar por qualquer outra coluna. Agrupando tarefas, você pode se concentrar em um subconjunto de tarefas. Cada grupo recolhível tem uma contagem de itens que são agrupados. Você também pode sinalizar rapidamente todos os itens no grupo clicando no botão **sinalizador** à direita do botão **recolher** .  
  
     ![Pilhas agrupadas na janela pilhas paralelas](../debugger/media/pdb-walkthrough-12a.png "PDB_Walkthrough_12A")  
  
     O último recurso da janela **tarefas paralelas** a ser examinado é o menu de atalho exibido quando você clica com o botão direito do mouse em uma tarefa.  
     ...
     Você pode congelar o thread subjacente de uma tarefa, ou tarefas, ou pode congelar todos os threads exceto o atribuído. Um thread congelado é representado na janela de **tarefas paralelas** como está na janela **threads** , por um ícone de *pausa* azul.  
