---
title: Visão geral de Diagnóstico de Gráficos | Microsoft Docs
ms.date: 11/15/2016
...
## <a name="directx-version-compatibility"></a>Compatibilidade de versão do DirectX
 O Diagnóstico de Gráficos dá suporte a aplicativos que usam o Direct3D 12, o Direct3D 11 e o Direct3D 10 e fornece suporte limitado para aplicativos que usam o Direct2D. Ele não oferece suporte a aplicativos que usam versões anteriores do Direct3D, DirectDraw ou outras APIs gráficas.

### <a name="windows-10-and-direct3d-12"></a>Windows 10 e Direct3D 12
 O Windows 10 apresenta a próxima versão do Direct3D, *Direct3D 12*, que é substancialmente diferente do Direct3D 10 e do Direct3D 11. Essas diferenças trazem o DirectX de volta ao alinhamento com o hardware de gráficos moderno e liberando seu potencial completo, mas também trazem grandes alterações de API e colocam maior responsabilidade no programador para gerenciar tempos de vida e contenção de recursos. Ter ótimas ferramentas de depuração é crucial para ajudar os programadores de gráficos a fazer essa transição, portanto Diagnóstico de Gráficos no Visual Studio 2015 dá suporte à Direct3D12 diretamente do início. Apesar das diferenças, Diagnóstico de Gráficos com o Direct3D 12 mantém a paridade de recursos com Diagnóstico de Gráficos com o Direct3D 11,2, com a exceção atual do recurso de análise de quadros. Em breve, o suporte para análise de quadros no Direct3D 12 será adicionado, seguido por novas ferramentas de diagnóstico para ajudá-lo a resolver novos tipos de bugs que você pode encontrar no Direct3D 12.

 O Windows 10 também mantém o suporte para versões anteriores do Direct3D e os jogos e aplicativos que dependem deles. Diagnóstico de Gráficos no Visual Studio 2015 continua a oferecer suporte ao Direct3D 10 e ao Direct3D 11 no Windows 10, bem como em Windows 8.1.

### <a name="windows-81-and-direct3d-112"></a>Windows 8.1 e Direct3D 11.2
 No [!INCLUDE[win81](../includes/win81-md.md)], o DirectX 11.2 apresenta novos recursos que incluem suporte para capturar informações gráficas ao longo do runtime. [!INCLUDE[win81](../includes/win81-md.md)] usa a nova captura baseada em tempo de execução, conhecida como *captura robusta*, exclusivamente para todas as versões do DirectX que [!INCLUDE[win81](../includes/win81-md.md)] dão suporte ao. A captura robusta também oferece suporte a novos recursos do Direct3D 11.2.
...
### <a name="diagnostics-capture-interface"></a>Interface de captura de diagnóstico
 ...
 Consulte [capturando informações de gráficos](../debugger/capturing-graphics-information.md) para obter mais informações.
...
### <a name="gpu-usage"></a>Uso de GPU
 ...
 Consulte [uso de GPU](../debugger/gpu-usage.md) para obter mais informações.
...
## <a name="graphics-analyzer"></a>Analisador de gráficos
 ...
 ![Todas as janelas do depurador de gráficos](../debugger/media/graphicsdebuggerwindows.png "GraphicsDebuggerWindows")
...
### <a name="graphics-log-document"></a>Documento de log de gráficos
 ...
 O [documento de log de gráficos](../debugger/graphics-log-document.md) também é o ponto de entrada para a ferramenta de análise de quadros, que ajuda você a entender o desempenho de um quadro, alterando a maneira como determinados aspectos da renderização são configurados e fornecendo resultados de benchmark para comparar com o original.
...
### <a name="graphics-state"></a>Estado gráfico
 ...
 A janela de [estado](../debugger/graphics-state.md) coleta as informações de estado relevantes para cada chamada de desenho em um único local para que seja mais fácil de localizar e também realça as alterações de estado ocorridas desde a chamada de desenho anterior.
...
### <a name="event-call-stack"></a>Pilha de chamadas do evento
 ...
 A janela [pilha de chamadas de evento](../debugger/graphics-event-call-stack.md) exibe a pilha de chamadas completa de cada evento de gráficos na lista de eventos e até mesmo permite que você salte para o código-fonte do aplicativo se as informações de depuração estiverem disponíveis. Essa é uma ferramenta poderosa para seguir um erro de onde ele aparece pela primeira vez, na GPU, de volta para onde ele se originou no código-fonte do seu aplicativo.
