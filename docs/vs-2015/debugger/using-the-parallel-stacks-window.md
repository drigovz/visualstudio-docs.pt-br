---
title: Usando a janela de pilhas paralelas | Microsoft Docs
ms.date: 11/15/2016
...
A janela de **pilhas paralelas** é útil quando você está Depurando aplicativos multithread. Sua **exibição threads** mostra informações de pilha de chamadas para todos os threads em seu aplicativo. Permite navegar entre os threads e os quadros da pilha nesses threads. No código gerenciado, a **exibição tarefas** mostra pilhas de chamadas de <xref:System.Threading.Tasks.Task?displayProperty=fullName> objetos. No código nativo, a **exibição tarefas** mostra pilhas de chamadas de [grupos de tarefas](https://msdn.microsoft.com/library/42f05ac3-2098-494a-ba84-737fcdcad077), [algoritmos paralelos](https://msdn.microsoft.com/library/045dca7b-4d73-4558-a44c-383b88a28473), [agentes assíncronos](https://msdn.microsoft.com/library/6cf6ccc6-87f1-4e14-af15-ea8ba58fef1a)e [tarefas leves](https://msdn.microsoft.com/library/9aba278c-e0c9-4ede-b7c6-fedf7a365d90).  
...
|F|Dica de ferramenta no contexto do método|Na exibição threads, ele mostra todos os threads em uma tabela semelhante à janela **threads** . Na exibição de tarefa, ele mostra todas as tarefas em uma tabela semelhante à janela **tarefas paralelas** .|  
  
 Além disso, a janela pilhas paralelas mostra o ícone de exibição de visão **panorâmica** no painel principal quando o grafo é muito grande para caber na janela. Você pode clicar no ícone para ver o gráfico inteiro na janela.  
...
## <a name="toolbar-controls"></a>Controles da barra de ferramentas  
 ...
 ![Barra de ferramentas na janela pilhas paralelas](../debugger/media/parallel-stackstoolbar.png "Parallel_StacksToolbar")  
  
|Letra de texto explicativo|Control|Descrição|  
|--------------------|-------------|-----------------|  
...
|B|Mostrar somente sinalizados|Mostra pilhas de chamadas somente para os threads sinalizados em outras janelas de depuração, como a janela **threads de GPU** e a janela de **inspeção paralela** .|  
...
|E|Ativar/Desativar Controle de Zoom|Mostra ou oculta o controle de zoom. Você também pode aplicar zoom pressionando CTRL e virando a roda do mouse, independentemente da visibilidade do controle de zoom ou usando **Ctrl + Shift + ' + '** para ampliar e **Ctrl + Shift + '-'** para reduzir. Pressionar **CTRL + F8** fará com que o zoom caiba na tela.|  
...
|Ir para Código Fonte|Navega até o local no código-fonte que corresponde ao quadro de pilhas que o usuário clicou com o botão direito.|  
...
## <a name="tasks-view"></a>Modo de Exibição de Tarefas  
 Se seu aplicativo estiver usando <xref:System.Threading.Tasks.Task?displayProperty=fullName> objetos (código gerenciado) ou `task_handle` objetos (código nativo) para expressar o paralelismo, você poderá usar a caixa de combinação na barra de ferramentas da janela pilhas paralelas para alternar para a *exibição tarefas*. O Modo de Exibição de Tarefas mostra as chamadas de pilhas de tarefas reais em vez de threads. O Modo de Exibição de Tarefas é diferente do Modo de Exibição de Threads da seguinte maneira:  
...
- Quando várias tarefas estiverem em um thread, as pilhas de chamadas dessas tarefas serão divididas em nós separados.  
  ...
  ![Exibição de tarefas na janela de pilhas paralelas](../debugger/media/parallel-tasksview.png "Parallel_TasksView")  
  
  Para ver a pilha de chamadas inteira, basta voltar para a exibição threads clicando com o botão direito do mouse em um registro de ativação e clicando em **ir para thread**.  
  ...
  ![Dicas de ferramenta na janela pilhas paralelas](../debugger/media/parallel-stack-tooltips.png "Parallel_Stack_Tooltips")  
...
## <a name="method-view"></a>Modo de Exibição do Método  
 ...
 ![Exibição de método na janela pilhas paralelas](../debugger/media/parallel-methodview.png "Parallel_MethodView")  
...
## <a name="see-also"></a>Consulte Também  
 [Walkthrough: Depurando um aplicativo paralelo](../debugger/walkthrough-debugging-a-parallel-application.md)   
