...
1. Localize **msvsmon.exe** no diretório que corresponde à sua versão do Visual Studio. Para o Visual Studio 2015:
...
6. Quando a configuração estiver concluída, a janela do depurador remoto será exibida.
   ...
   ** \<Visual Studio installation directory> \Common7\IDE\Remote Debugger \\<x86, x64 ou Appx\msvsmon.exe**.  
...
## <a name="remote-debug-an-aspnet-application"></a>Depuração remota de um aplicativo ASP.NET  
 ...
 Se você estiver depurando um aplicativo ASP.NET Core, consulte [publicando no IIS](https://docs.asp.net/en/latest/publishing/iis.html). Etapas diferentes são necessárias para publicar um ASP.NET Core no IIS. Depois de publicar um aplicativo de ASP.NET Core com êxito, você pode depurá-lo remotamente, [assim como outros aplicativos ASP.net](../debugger/remote-debugging-aspnet-on-a-remote-iis-7-5-computer.md), exceto que o processo que você precisa anexar é dnx.exe em vez de w3wp.exe.

## <a name="remote-debug-a-visual-c-project"></a>Depuração remota de um projeto Visual C++  
...
## <a name="remote-debug-a-visual-c-or-visual-basic-project"></a>Depuração remota de um projeto do Visual C# ou Visual Basic  
...
## <a name="remote-debugging-on-windows-store-and-azure-apps"></a><a name="bkmk_winstoreAzure"></a> Depuração remota na Windows Store e aplicativos do Azure  
...
- [Depuração de um Serviço de Nuvem ou Máquina Virtual no Visual Studio](../azure/vs-azure-tools-debug-cloud-services-virtual-machines.md)  
