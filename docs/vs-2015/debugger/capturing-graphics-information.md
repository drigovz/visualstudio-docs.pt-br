...
- Na barra de ferramentas de **gráficos** , escolha o botão **Iniciar diagnóstico** .  
...
- No Visual Studio, na barra de ferramentas de **gráficos** , escolha o botão **capturar quadro** ![ícone de captura de gráficos](../debugger/media/debuggingdirectxgraphics.png "DebuggingDirectXGraphics").  
...
- Na interface de captura do Visual Studio, escolha o botão **capturar quadro** localizado acima da linha do tempo da **sessão de diagnóstico** ou selecione o botão grande do quadro de captura localizado abaixo da rota de **retransmissão** de **quadros por segundo** e à direita de todos os quadros capturados anteriormente. Ambos os botões estão realçados na imagem abaixo.  
...
2. Na lista da categoria opções à esquerda, escolha o Diagnóstico de Gráficos e configure as opções de Diagnóstico de Gráficos que você deseja.  
     ...
     **Parar captura se for encontrado algum erro nas camadas de SDK**  
     Marque esta caixa para parar a captura imediatamente se forem encontrados erros.  
...
## <a name="capturing-graphics-information-remotely"></a>Capturando informações de gráficos remotamente  
 ...
 Como você configura o projeto para depuração remota depende do tipo de aplicativo que está sendo desenvolvido e da linguagem de programação que está sendo usada. Para obter informações sobre como configurar a depuração remota para um aplicativo da Windows Store, consulte [executar aplicativos da Windows Store em um computador remoto](../debugger/run-windows-store-apps-on-a-remote-machine.md). Para obter informações sobre como configurar a depuração remota para um aplicativo de área de trabalho do Windows, consulte [Configurar a depuração remota para um projeto do Visual Studio](https://msdn.microsoft.com/library/ec332dc4-400a-498b-a0e6-c8dcf10fef8a).  
