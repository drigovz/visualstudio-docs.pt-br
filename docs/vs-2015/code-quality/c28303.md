...
aviso C28303: para o parâmetro de referência do C++ <parameter_name>, um `_Deref_` operador ambíguo foi encontrado em \<annotation> .  
  
 Esse aviso é semelhante ao aviso C28302 e é relatado quando um nível extra de `_Deref_` é usado em um parâmetro.  
  
 O SAL2 não exige o uso de um nível extra de `_Deref_` ao lidar com parâmetros de referência. Essa anotação específica é ambígua sobre qual nível de desreferência deve ser anotado. Pode ser necessário usar `_At_` o para fazer referência ao objeto específico a ser anotado.  
