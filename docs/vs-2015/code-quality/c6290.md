...
- A expressão é incorreta entre parênteses:  
   ...
   Como o resultado de `!` é booliano (zero ou um), uma tentativa de testar que duas variáveis têm bits em comum só acabará testando se o menor bit está presente no lado direito: `((!8) & 1) == 0` .  
...
- O operador Binary `&` está incorreto e, em vez disso, deve ser `&&` :  
   ...
   Embora `&` às vezes possa ser trocado com `&&` , ele não é equivalente porque força a avaliação do lado direito da expressão. Certos efeitos colaterais nesse tipo de expressão podem ser terminal.  
