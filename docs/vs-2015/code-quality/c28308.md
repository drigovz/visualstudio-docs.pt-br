...
- Um offset ( `n` ) relativo ao parâmetro Format-String.  
  
  No segundo formulário, o primeiro parâmetro Format-String é o `n` argumento-ésimo após a cadeia de caracteres de formato. Se `n` for zero, uma elipse será especificada como o parâmetro. A especificação de um deslocamento de zero sem especificar as reticências como o primeiro parâmetro Format-String causará um erro.
