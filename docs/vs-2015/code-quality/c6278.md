...
aviso C6278: \<variable> é alocado com New [] de matriz, mas excluído com a exclusão escalar. Destruidores não serão chamados  
 ...
 Esse aviso aparece apenas no código C++ e indica que a função de chamada tem memória inconsistente alocada com o operador **New []** da matriz, mas liberou-a com o operador de **exclusão** escalar. Esse é um comportamento indefinido de acordo com o padrão C++ e a implementação de Microsoft Visual C++. Há pelo menos três motivos pelos quais isso provavelmente causará problemas:  
