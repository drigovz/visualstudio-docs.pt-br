...
## <a name="how-to-fix-violations"></a>Como Corrigir Violações
 Examine seu código para certificar-se de que ambas as ações são necessárias. Se ambas as ações forem necessárias, verifique se a ação no nível do método inclui a segurança especificada no nível de tipo. Por exemplo, se o tipo exigir permissão `X`, e seu método também precisar solicitar permissão `Y`, o método deverá solicitar explicitamente `X` e `Y`.
 ...
 **[nenhuma permissão de leitura (exigida por método)] não pôde acessar pessoal informações: falha na solicitação.**
