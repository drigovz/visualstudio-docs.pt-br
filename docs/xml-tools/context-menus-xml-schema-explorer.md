...
|**Mostrar todas as referências de saída**|Para cada arquivo no namespace selecionado, localiza e ressalta o seguinte:<br /><br /> -Todos os namespaces referenciados em instruções de `schemaLocation` importação sem um atributo.<br />-Todos os arquivos em namespaces diferentes do selecionado que são especificados no `schemaLocation` atributo nas instruções import e include.|
...
|**Mostrar todas as referências de saída**|Localiza e realces o seguinte:<br /><br /> -Todos os namespaces especificados nos atributos de namespace de todas as instruções de importação que não `schemaLocation` têm o atributo.<br />-Todos os arquivos especificados nos `schemaLocation` atributos de todas as instruções import e include.|
