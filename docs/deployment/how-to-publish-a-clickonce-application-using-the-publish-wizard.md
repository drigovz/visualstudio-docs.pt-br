---
title: Publicar o aplicativo ClickOnce usando o assistente de publicação
description: Saiba mais sobre como usar o assistente de publicação para disponibilizar seu aplicativo ClickOnce para os usuários, incluindo quais propriedades de publicação usar.
...
## <a name="to-publish-to-a-file-share-or-path"></a>Para publicar em um caminho ou compartilhamento de arquivos

1. No **Gerenciador de Soluções**, selecione o projeto do aplicativo.

2. No menu **Compilar** , clique em **publicar** *ProjectName*.

    O Assistente de Publicação será exibido.

3. Na página **onde deseja publicar o aplicativo?** , insira um endereço de servidor FTP válido ou um caminho de arquivo válido usando um dos formatos mostrados e clique em **Avançar**.
...
4. Na página **Como os usuários instalarão o aplicativo?**, selecione a localização em que os usuários acessarão para instalar o aplicativo:
   ...
   - Se os usuários forem instalar de um site, clique em **De um site** e insira a URL que corresponde ao caminho de arquivo inserido na etapa anterior. Clique em **Próximo**. (Essa opção geralmente é usada ao especificar um endereço FTP no local de publicação. O download direto do FTP não tem suporte. Portanto, é necessário inserir uma URL aqui.)
...
3. Na página **Em que local você deseja publicar o aplicativo?**, insira o caminho de arquivo ou localização FTP no qual o aplicativo será publicado, por exemplo, *d:\deploy*. Em seguida, clique em **Avançar** para continuar.
