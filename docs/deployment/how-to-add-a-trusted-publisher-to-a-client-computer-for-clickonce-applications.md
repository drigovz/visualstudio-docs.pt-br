---
title: Adicionar um fornecedor confiável ao computador cliente para aplicativos ClickOnce
ms.date: 11/04/2016
ms.topic: conceptual
dev_langs:
- VB
...
Com a implantação de aplicativo confiável, você pode configurar computadores cliente para que seus aplicativos de [!INCLUDE[ndptecclick](../deployment/includes/ndptecclick_md.md)] sejam executados com um nível mais alto de confiança sem avisar o usuário. Os procedimentos a seguir mostram como usar a ferramenta de linha de comando CertMgr. exe para adicionar o certificado de um editor ao repositório de editores confiáveis em um computador cliente.
...
> [!NOTE]
> Você deve adicionar certificados dessa maneira em cada computador cliente ao qual planeja implantar um [!INCLUDE[ndptecclick](../deployment/includes/ndptecclick_md.md)] aplicativo que requer permissões elevadas. Você adiciona os certificados manualmente ou por meio de um aplicativo implantado em seus clientes. Você só precisa configurar esses computadores uma vez, após o qual é possível implantar qualquer número de aplicativos [!INCLUDE[ndptecclick](../deployment/includes/ndptecclick_md.md)] assinados com o mesmo certificado.
