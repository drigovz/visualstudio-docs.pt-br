...
1. Na caixa de diálogo novo projeto, procure e selecione o modelo de **projeto Web Django de pesquisas** , chame o projeto "DjangoPolls" e selecione **OK**.
...
1. Quando o Python concluir a configuração do ambiente virtual, siga as instruções no *readme.html* exibido para inicializar o banco de dados e criar um superusuário do Django (ou seja, um administrador). As etapas são para primeiro clicar com o botão direito do mouse no projeto **DjangoPolls** em **Gerenciador de soluções**, selecionar o comando **python**  >  **Django migrar** e clicar com o botão direito do mouse no projeto novamente, selecionar o comando **Python**  >  **Django criar superusuário** e seguir os prompts. (Se você tentar criar um superusuário pela primeira vez, verá um erro porque o banco de dados não foi inicializado).

1. Defina o projeto **DjangoPolls** como o padrão para a solução do Visual Studio clicando com o botão direito do mouse no projeto em **Gerenciador de soluções** e selecionando **definir como projeto de inicialização**. O projeto de inicialização, mostrado em negrito, é o que é executado quando você inicia o depurador.
...
1. Para aplicar os scripts ao banco de dados real, clique com o botão direito do mouse no projeto novamente e selecione **Python**  >  **Django migrar**.
...
1. Salve o arquivo, clique com o botão direito do mouse no projeto **DjangoPolls** em **Gerenciador de soluções** e selecione o comando **python**  >  **Django Make Migrations** .
1. Selecione o comando **projeto**  >  **Mostrar todos os arquivos** para ver o script gerado recentemente na pasta **migrações** , cujo nome começa com **002_auto_**. Clique com o botão direito no arquivo e selecione **Include In Project**. Em seguida, você pode selecionar o **projeto**  >  **Mostrar todos os arquivos** novamente para restaurar o modo de exibição original. (Veja a segunda pergunta abaixo para obter detalhes sobre esta etapa).
...
1. Clique com o botão direito do mouse no projeto do Visual Studio novamente e selecione **Python**  >  **Django migrar** para aplicar as alterações ao banco de dados.
...
Resposta: se os modelos não corresponderem ao que há no banco de dados, o Django falhará em tempo de execução com as exceções apropriadas. Por exemplo, se você esquecer de migrar a alteração de modelo mostrada na seção anterior, verá um erro **sem essa coluna: app_poll. Author**:
