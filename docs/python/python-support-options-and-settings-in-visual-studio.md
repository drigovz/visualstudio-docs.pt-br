...
Para exibir as opções do Python, use o comando de menu opções de **ferramentas**  >   , verifique se a opção **Mostrar todas as configurações** está selecionada e, em seguida, navegue até **Python**:
...
Também há opções adicionais específicas do Python na guia Avançado do Python **Editor de texto**  >    >   e na guia   >  **fontes e cores** do ambiente dentro do grupo **Editor de texto** .
...
(**Ferramentas**  >  **Opções**  >  do Guia **Python** .)
...
| **Gerar automaticamente o BD de conclusão no primeiro uso** | Ativado | *Aplica-se ao Visual Studio 2017 versão 15.5 e anteriores e a versões posteriores ao usar um banco de dados do IntelliSense.* Prioriza a conclusão do banco de dados para uma biblioteca quando você escreve código que a usa. Confira mais informações na [guia IntelliSense](python-environments-window-tab-reference.md?view=vs-2017&preserve-view=true#intellisense-tab). |
| **Ignorar variáveis PYTHONPATH de todo o sistema** | Ativado | PYTHONPATH é ignorado por padrão porque o Visual Studio fornece um meio mais direto para especificar caminhos de pesquisa em projetos e ambientes. Confira [Caminhos de pesquisa](search-paths.md) para obter detalhes. |
| **Atualizar os caminhos de pesquisa ao adicionar arquivos vinculados** | Ativado | Quando definido, adicionar um [arquivo vinculado](managing-python-projects-in-visual-studio.md#linked-files) a um projeto atualiza os [caminhos de pesquisa](search-paths.md) para que o IntelliSense possa incluir o conteúdo da pasta do arquivo vinculado em seu banco de dados de conclusão. Desmarque esta opção para excluir o conteúdo do banco de dados de conclusão. |
...
![Captura de tela da caixa de diálogo opções de Ferramentas do Visual Studio com Conda selecionado nas opções do Python e no campo caminho executável do Conda mostrado no painel direito.](media/options-conda.png)

::: moniker-end

## <a name="debugging-options"></a>Opções de depuração

(**Ferramentas**  >  **Opções**  >  do **Python**  >  Guia **depuração** .)

| Opção | Padrão | Descrição |
| --- | --- | --- |
| **Perguntar antes de executar quando houver erros** | Ativado | Quando definido, solicita que você confirme que deseja executar o código que contém erros. Desmarque esta opção para desabilitar o aviso. |
...
| **Mostrar o valor retornado da função** | Ativado | *Somente Visual Studio 2019.* Exibe os valores retornados de função na janela **Locals**, em seguida, passa uma chamada de função no depurador (F10) |
...
(**Ferramentas**  >  **Opções**  >  do **Python**  >  Guia **diagnóstico** .)
...
(**Ferramentas**  >  **Opções**  >  do **Python**  >  Guia **interativa do Windows** .)
...
![Captura de tela da caixa de diálogo opções de Ferramentas do Visual Studio com o servidor de idioma selecionado nas opções de Python e as opções de servidor de idioma mostradas no painel direito.](media/options-language-server.png)

::: moniker-end

## <a name="advanced-python-editor-options"></a>Opções avançadas de editor de Python

(**Ferramentas**  >  **Opções**  >  do **Editor**  >  de texto **Python**  >  Guia **avançado** .)
...
| **Lista de filtro com base na cadeia de pesquisa** | Ativado | Aplica a filtragem de sugestões de preenchimento conforme você digita (o padrão é marcado). |
| **Mostrar automaticamente conclusões para todos os identificadores** | Ativado | Desmarque esta opção para desabilitar as conclusões no editor e nas janelas **Interativas**. |
...
| **Inserir a conclusão atual de confirmação** | Ativado | Quando definido, a tecla **Enter** escolhe e aplica a conclusão escolhida no momento, assim como acontece com os caracteres acima (mas obviamente, não há um caractere para **Enter** portanto, ele não pode entrar diretamente para essa lista!). |
...
| Opção | Padrão | Descrição |
| --- | --- | --- |
| **Entrar no modo de estrutura de tópicos quando os arquivos forem abertos** | Ativado | Ative automaticamente o recurso de estrutura de tópicos do Visual Studio no editor ao abrir o arquivo de código do Python. |
| **Colar prompts REPL removidos** | Ativado | Remove **>>>** e **...** do texto colado, permitindo a transferência fácil de código da janela **interativa** para o editor. Desmarque essa opção se você precisar manter esses caracteres ao colar de outras fontes. |
| **Nomes de cores com base em tipos** | Ativado | Habilita as cores de sintaxe no código do Python. |
...
(**Ambiente**  >  Guia **fontes e cores** no grupo **Editor de texto** .)

Os nomes das opções do Python são prefixadas com **Python** e são autoexplicativas. A fonte padrão para todos os temas de cores do Visual Studio é de 10 pt consolas regular (não negrito). As cores padrão variam de acordo com o tema. Normalmente, você altera uma fonte ou a cor se tiver dificuldade para ler texto com as configurações padrão.
