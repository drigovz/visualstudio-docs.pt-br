...
|**Coordenada de textura**|As coordenadas de textura do pixel atual.<br /><br /> As coordenadas de textura são determinadas pela interpolação entre os atributos de coordenadas de textura de vértices próximos. É possível considerar isso como a posição do pixel atual no espaço de textura.<br /><br /> **Saída:**<br /><br /> `Output`: `float2`<br /> As coordenadas de textura.|Nenhuma|
