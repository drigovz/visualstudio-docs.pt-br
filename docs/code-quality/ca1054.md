...
Para corrigir uma violação dessa regra, altere o parâmetro para um tipo de <xref:System.Uri>; Essa é uma alteração significativa. Como alternativa, forneça uma sobrecarga do método que usa um parâmetro <xref:System.Uri>; Essa é uma alteração sem interrupção.
