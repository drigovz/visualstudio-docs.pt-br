...
Um membro em um assembly que não está marcado com a <xref:System.Runtime.CompilerServices.RuntimeCompatibilityAttribute> ou está marcado `RuntimeCompatibility(WrapNonExceptionThrows = false)` contém um bloco catch que manipula <xref:System.Exception?displayProperty=fullName> e não contém um bloco catch geral imediatamente após. Essa regra ignora [!INCLUDE[vbprvb](../code-quality/includes/vbprvb_md.md)] assemblies.
...
Para corrigir uma violação dessa regra quando a intenção é capturar todas as exceções, substitua ou adicione um bloco catch geral ou marque o `RuntimeCompatibility(WrapNonExceptionThrows = true)`do assembly. Se as permissões forem removidas no bloco catch, duplique a funcionalidade no bloco catch geral. Se não for a intenção de lidar com todas as exceções, substitua o bloco catch que manipula <xref:System.Exception> com blocos catch que lidam com tipos de exceção específicos.
...
[CA1031: não capturar tipos de exceção gerais](../code-quality/ca1031.md)
