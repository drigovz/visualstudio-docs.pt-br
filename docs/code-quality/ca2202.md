...
As instruções `using` aninhadas (`Using` em Visual Basic) podem causar violações do aviso CA2202. Se o recurso IDisposable da instrução de `using` interna aninhada contiver o recurso da instrução `using` externa, o método `Dispose` do recurso aninhado liberará o recurso contido. Quando essa situação ocorre, o método `Dispose` da instrução `using` externa tenta descartar seu recurso por uma segunda vez.

No exemplo a seguir, um objeto <xref:System.IO.Stream> que é criado em uma instrução using externa é liberado no final da instrução using interna no método Dispose do objeto <xref:System.IO.StreamWriter> que contém o objeto `stream`. No final da instrução de `using` externa, o objeto `stream` é liberado uma segunda vez. A segunda versão é uma violação do CA2202.
...
Para resolver esse problema, use um `try`/bloco de `finally` em vez da instrução de `using` externa. No bloco de `finally`, verifique se o recurso de `stream` não é nulo.
...
> [!TIP]
> A sintaxe de `?.` acima é o [operador NULL-Conditional](/dotnet/csharp/language-reference/operators/member-access-operators#null-conditional-operators--and-).
