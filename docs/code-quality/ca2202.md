...
Um método <xref:System.IDisposable.Dispose%2A> implementado corretamente pode ser chamado várias vezes sem gerar uma exceção. No entanto, isso não é garantido e para evitar a geração de um <xref:System.ObjectDisposedException?displayProperty=fullName>, você não deve chamar <xref:System.IDisposable.Dispose%2A> mais de uma vez em um objeto.
...
Para resolver esse problema, use um bloco `try` @ no__t-1 @ no__t-2 em vez da instrução `using` externa. No bloco `finally`, verifique se o recurso `stream` não é nulo.
