...
- Um valor de <xref:System.Security.Authentication.SslProtocols?displayProperty=nameWithType> seguro, mas codificado, foi referenciado.
- Um valor inteiro que representa uma versão de protocolo seguro foi atribuído a uma variável <xref:System.Security.Authentication.SslProtocols>, usado como um valor de retorno de <xref:System.Security.Authentication.SslProtocols> ou usado como um argumento <xref:System.Security.Authentication.SslProtocols>.
