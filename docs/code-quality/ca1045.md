...
Passar tipos por referência (usando `out` ou `ref`) requer experiência com ponteiros, compreendendo como tipos de valor e tipos de referência diferem e manipulando métodos que têm vários valores de retorno. Além disso, a diferença entre os parâmetros `out` e `ref` não é amplamente compreendida.
...
Embora os valores de retorno sejam comuns e altamente usados, a aplicação correta dos parâmetros `out` e `ref` exige habilidades intermediárias de design e codificação. Os arquitetos de biblioteca que projetam um público geral não devem esperar que os usuários façam o mestre de trabalho com `out` ou `ref` parâmetros.

> [!NOTE]
> Quando você trabalha com parâmetros que são estruturas grandes, os recursos adicionais necessários para copiar essas estruturas podem causar um efeito de desempenho quando você passa por valor. Nesses casos, você pode considerar o uso de `ref` ou `out` parâmetros.
...
## <a name="example"></a>Exemplo
O aplicativo a seguir ilustra a experiência do usuário. A chamada para a biblioteca recriada (`UseTheSimplifiedClass` método) é mais simples, e as informações retornadas pelo método são facilmente gerenciadas. A saída dos dois métodos é idêntica.
