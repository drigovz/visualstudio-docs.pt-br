...
Um tipo público ou protegido em um assembly com o atributo <xref:System.Security.AllowPartiallyTrustedCallersAttribute?displayProperty=fullName> herda de um tipo declarado em um assembly que não tem o atributo.
...
Um tipo parcialmente confiável `X` pode herdar de `T1`, o que fornece acesso a membros herdados declarados em `T2`. Como `T2` não tem o atributo APTCA, seu tipo derivado imediato (`T1`) deve atender a uma demanda de herança para confiança total; `T1` tem confiança total e, portanto, satisfaz essa verificação. O risco de segurança é porque `X` não participa da demanda de herança que protege `T2` de subclasses não confiáveis. Por esse motivo, os tipos com o atributo APTCA não devem estender tipos que não tenham o atributo.
...
O segundo assembly, representado por `T1` na discussão anterior, é totalmente confiável e permite chamadores parcialmente confiáveis.
