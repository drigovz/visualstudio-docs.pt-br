...
Antes de .NET Framework 2,0, para passar informações personalizadas para o manipulador de eventos, um novo delegado tinha de ser declarado que especificou uma classe que era derivada da classe <xref:System.EventArgs?displayProperty=fullName>. No .NET Framework 2,0 e versões posteriores, o delegado <xref:System.EventHandler%601?displayProperty=fullName> genérico permite que qualquer classe derivada de <xref:System.EventArgs> seja usada junto com o manipulador de eventos.
