...
Métodos de invocação de plataforma, como aqueles que são marcados usando o atributo <xref:System.Runtime.InteropServices.DllImportAttribute?displayProperty=fullName> ou métodos que são definidos usando a palavra-chave `Declare` no [!INCLUDE[vbprvb](../code-quality/includes/vbprvb_md.md)], acessam código não gerenciado. Esses métodos devem estar em uma das seguintes classes:
...
Essas classes são declaradas como `internal` (`Friend`, em Visual Basic) e declaram um Construtor privado para impedir que novas instâncias sejam criadas. Os métodos nessas classes devem ser `static` e `internal` (`Shared` e `Friend` no Visual Basic).
