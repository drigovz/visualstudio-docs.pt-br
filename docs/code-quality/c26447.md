...
C++Principais diretrizes: [F. 6: Se sua função não puder ser lançada, declare-a noexcept @ no__t-0.
...
- A regra se concentra apenas em chamadas de função. Ele sinaliza os destinos que não são **constexpr** e que podem gerar exceções potencialmente; em outras palavras, elas não são marcadas explicitamente como não lançadas usando **noexcept**, **overdeclspec (nothrow)** , **throw ()** .
...
- O verificador também ignora tipos especiais de funções de destino que devem ser implementadas como **noexcept**; Essa regra é imposta por [C26439 SPECIAL_NOEXCEPT](c26439.md).
