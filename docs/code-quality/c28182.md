...
ms.locfileid: "72015114"
---
# <a name="c28182"></a>C28182
C28182 de aviso: Desreferenciando ponteiro nulo.

 **Informações adicionais**: *\<pointer1 >* contém o mesmo valor nulo que *\<pointer2 >* @no__t *-6note >*
 ...
 *\<pointer2 >* é confirmado como potencialmente nulo. *\<pointer1 >* contém o mesmo valor que *pointer2* e está sendo cancelado. Como esses ponteiros podem estar em locais muito diferentes no código, ambos são relatados para que você possa determinar por que a ferramenta de análise de código está relatando esse aviso.
 ...
 Se uma instância anterior não confirmada da condição existir, então *\<note >* será substituído por este texto: "Consulte *a linha \<number >* para um local anterior em que isso pode ocorrer."

## <a name="example"></a>Exemplo
 O exemplo a seguir mostra o código que pode fazer com que a ferramenta de análise de código gere essa mensagem de aviso. Neste exemplo, a ferramenta de análise de código determina que `pNodeFree` é nula na instrução `if` e o caminho do código no corpo do `if` é obtido. No entanto, como `nBlockSize` é potencialmente zero, o corpo da instrução `for` não é executado e `pNodeFree` é deixado sem modificações. em seguida, `pNodeFree` é atribuído a `pNode`, e `pNode` é usado enquanto uma desreferência nula pode ocorrer.
...
## <a name="example"></a>Exemplo
 Uma maneira de corrigir o exemplo anterior é verificar `pNode` para zero antes de desreferenciá-lo para que uma desreferência nula seja AVERT. O código a seguir mostra essa correção.
