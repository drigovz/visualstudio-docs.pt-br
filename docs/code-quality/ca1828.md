---
title: 'CA1828: não use CountAsync/LongCountAsync quando AnyAsync puder ser usado'
ms.date: 04/24/2020
ms.topic: reference
f1_keywords:
...
ms.locfileid: "82172495"
---
# <a name="ca1828-do-not-use-countasynclongcountasync-when-anyasync-can-be-used"></a>CA1828: não use CountAsync/LongCountAsync quando AnyAsync puder ser usado
...
Essa regra sinaliza as <xref:Microsoft.EntityFrameworkCore.EntityFrameworkQueryableExtensions.CountAsync%2A> chamadas <xref:Microsoft.EntityFrameworkCore.EntityFrameworkQueryableExtensions.LongCountAsync%2A> de método LINQ e usadas para verificar se a coleção tem pelo menos um elemento. Essas chamadas de método exigem a enumeração de toda a coleção para calcular a contagem. A mesma verificação é mais rápida com <xref:Microsoft.EntityFrameworkCore.EntityFrameworkQueryableExtensions.AnyAsync%2A> o método, pois evita a enumeração da coleção.
...
Para corrigir uma violação, substitua a <xref:Microsoft.EntityFrameworkCore.EntityFrameworkQueryableExtensions.CountAsync%2A> chamada <xref:Microsoft.EntityFrameworkCore.EntityFrameworkQueryableExtensions.LongCountAsync%2A> de método ou pelo <xref:Microsoft.EntityFrameworkCore.EntityFrameworkQueryableExtensions.AnyAsync%2A> método. Por exemplo, os dois trechos de código a seguir mostram uma violação da regra e como corrigi-la:
