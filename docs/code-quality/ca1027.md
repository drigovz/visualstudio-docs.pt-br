...
No exemplo a seguir, `DaysEnumNeedsFlags` é uma enumeração que atende aos requisitos para usar <xref:System.FlagsAttribute>, mas não tem. A enumeração de `ColorEnumShouldNotHaveFlag` não tem valores que são potências de dois, mas especificam <xref:System.FlagsAttribute>incorretamente. Isso viola a regra [CA2217: não marque enums com FlagsAttribute](../code-quality/ca2217.md).
