...
Para corrigir uma violação dessa regra, Capture uma exceção mais específica ou relance a exceção geral como a última instrução no bloco de `catch`.
...
Formatos de nome de tipo permitidos no valor de opção (separados por ' | '):
  ...
  - Nomes totalmente qualificados no [formato de ID de documentação](https://github.com/dotnet/csharplang/blob/master/spec/documentation-comments.md#id-string-format) do símbolo com um prefixo `T:`.
...
O exemplo a seguir mostra um tipo que viola essa regra e um tipo que implementa corretamente o bloco de `catch`.
