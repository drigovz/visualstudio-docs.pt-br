---
title: 'CA5378: não desabilitar ServicePointManagerSecurityProtocols'
...
ms.locfileid: "74809837"
---
# <a name="ca5378-do-not-disable-servicepointmanagersecurityprotocols"></a>CA5378: não desabilitar ServicePointManagerSecurityProtocols
...
Uma chamada de método <xref:System.AppContext.SetSwitch%2A?displayProperty=nameWithType> define `Switch.System.ServiceModel.DisableUsingServicePointManagerSecurityProtocols` como `true`.

Por padrão, essa regra analisa toda a base de código, mas é [configurável](#configurability).

## <a name="rule-description"></a>Descrição da regra

Definir `Switch.System.ServiceModel.DisableUsingServicePointManagerSecurityProtocols` como `true` limita as conexões de TLS (segurança de camada de transporte) do Windows Communication Framework (WCF) usando o TLS 1,0. Essa versão do TLS será preterida. Para obter mais informações, consulte [práticas recomendadas de TLS (Transport Layer Security) com o .NET Framework](/dotnet/framework/network-programming/tls#switchsystemservicemodeldisableusingservicepointmanagersecurityprotocols).

## <a name="how-to-fix-violations"></a>Como corrigir violações

- Se seu aplicativo se destina .NET Framework v 4.7 ou posterior, você pode remover a chamada de método <xref:System.AppContext.SetSwitch%2A?displayProperty=nameWithType> ou definir o valor do comutador como `false`.
