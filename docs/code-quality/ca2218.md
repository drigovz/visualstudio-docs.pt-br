...
Para corrigir uma violação dessa regra, forneça uma implementação de <xref:System.Object.GetHashCode%2A>. Para um par de objetos do mesmo tipo, você deve garantir que a implementação retorne o mesmo valor se a implementação de <xref:System.Object.Equals%2A> retornar `true` para o par.
