---
title: 'CA2007: não aguardando diretamente uma tarefa'
ms.date: 03/08/2019
ms.topic: reference
f1_keywords:
- CA2007
...
ms.locfileid: "75584615"
---
# <a name="ca2007-do-not-directly-await-a-task"></a>CA2007: não aguardando diretamente uma tarefa
...
Para corrigir violações, chame <xref:System.Threading.Tasks.Task.ConfigureAwait%2A> no <xref:System.Threading.Tasks.Task>esperado. Você pode passar `true` ou `false` para o parâmetro `continueOnCapturedContext`.
...
- Chame `ConfigureAwait(false)` na tarefa para agendar as continuaçãos para o pool de threads, evitando assim um deadlock no thread da interface do usuário. A passagem de `false` é uma boa opção para bibliotecas independentes de aplicativo.
