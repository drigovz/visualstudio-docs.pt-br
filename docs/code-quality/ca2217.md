...
O código a seguir mostra uma enumeração, `Color`, que contém o valor 3. 3 não é uma potência de dois ou uma combinação de qualquer um dos valores definidos. A enumeração de `Color` não deve ser marcada com <xref:System.FlagsAttribute>.
