---
title: Status de porta da regra de FxCop
ms.date: 05/21/2019
ms.topic: reference
helpviewer_keywords:
- fxcop rules
...
ms.locfileid: "66038642"
---
# <a name="fxcop-rule-port-status"></a>Status de porta da regra de FxCop

Se você tiver usado a análise de código estático em uma versão anterior do Visual Studio, você pode estar se perguntando quais essas regras estão disponíveis na implementação atual como [analisadores FxCop](install-fxcop-analyzers.md). Esta página lista as regras que são transportadas, bem como aqueles que ainda não foi compilado e se há planos para transportá-los.

## <a name="ported-rules"></a>Regras de porta

O [página de documentação geradas automaticamente](https://github.com/dotnet/roslyn-analyzers/blob/master/src/Microsoft.CodeAnalysis.FxCopAnalyzers/Microsoft.CodeAnalysis.FxCopAnalyzers.md) os analisadores de roslyn repositório tem a lista mais atualizada de regras que foram portadas para analisadores FxCop. Essa página também tem informações adicionais, como se a regra é habilitada por padrão e se ele tem um associado *correção de código*. ([Correções de código](../ide/quick-actions.md) estão em um único clique correções disponíveis no menu do ícone de lâmpada no Visual Studio.)

A partir da data desta página, a lista de FxCop regras que foram portadas para [analisadores FxCop](install-fxcop-analyzers.md) inclui:
...
[CA1016](ca1016-mark-assemblies-with-assemblyversionattribute.md) | Marcar assemblies com versão do assembly
...
[CA1028](ca1028-enum-storage-should-be-int32.md) | Armazenamento de enum deve ser Int32
...
[CA1043](ca1043-use-integral-or-string-argument-for-indexers.md) | Usar Integral ou argumento de cadeia de caracteres para indexadores
...
[CA1052](ca1052-static-holder-types-should-be-sealed.md) | Tipos de suporte estático devem ser estáticos ou NotInheritable (consulte a observação após esta tabela)
[CA1053](ca1053-static-holder-types-should-not-have-constructors.md) | Tipos de espaços reservados estáticos não devem ter construtores (consulte a observação após esta tabela)
[CA1054](ca1054-uri-parameters-should-not-be-strings.md) | Parâmetros de URI não devem ser cadeias de caracteres
[CA1055](ca1055-uri-return-values-should-not-be-strings.md) | Valores não devem ser cadeias de caracteres de retorno de URI
[CA1056](ca1056-uri-properties-should-not-be-strings.md) | Propriedades de URI não devem ser cadeias de caracteres
[CA1058](ca1058-types-should-not-extend-certain-base-types.md) | Tipos não devem estender determinados tipos base
[CA1060](ca1060-move-p-invokes-to-nativemethods-class.md) | Mover pinvokes à classe de métodos nativos
...
CA1066 | Tipo de {0} deve implementar IEquatable<T> porque ela substitui igual a
CA1067 | Substituir Object.Equals(object) ao implementar IEquatable<T>
CA1068 | Parâmetros de CancellationToken devem vir por últimos
CA1200 | Evite usar cref marcas com um prefixo
...
[CA1309](ca1309-use-ordinal-stringcomparison.md) | Usar a comparação de cadeia de caracteres ordinais
...
[CA1507](ca1507.md) | Usar nameof para expressar a nomes de símbolos
CA1508 | Evite código condicional dead
...
[CA1720](ca1720-identifiers-should-not-contain-type-names.md) | Identificador contém o nome do tipo
[CA1721](ca1721-property-names-should-not-match-get-methods.md) | Nomes de propriedades não devem corresponder a métodos get
[CA1724](ca1724-type-names-should-not-match-namespaces.md) | Nomes de tipo não devem corresponder a namespaces
...
[CA1816](ca1816-call-gc-suppressfinalize-correctly.md) | Métodos Dispose devem chamar SuppressFinalize
...
[CA1821](ca1821-remove-empty-finalizers.md) | Remova finalizadores vazios
...
CA1825 | Evite alocações de matriz de comprimento zero.
CA1826 | Não use métodos de Enumerable em coleções indexáveis. Em vez disso, use a coleção diretamente
...
[CA2007](ca2007-do-not-directly-await-task.md) | Considere a possibilidade de chamar o ConfigureAwait da tarefa aguardada
CA2008 | Não crie tarefas sem passar um TaskScheduler
CA2009 | Não chame ToImmutableCollection em um valor de ImmutableCollection
CA2010 | Sempre consumir o valor retornado por métodos marcados com PreserveSigAttribute
...
[CA2200](ca2200-rethrow-to-preserve-stack-details.md) | Relançar para preservar detalhes da pilha.
...
[CA2224](ca2224-override-equals-on-overloading-operator-equals.md) | Substituir Equals ao sobrecarregar operador equals
...
[CA2231](ca2231-overload-operator-equals-on-overriding-valuetype-equals.md) | Sobrecarregar operador equals ao substituir o valor de tipo é igual a
[CA2234](ca2234-pass-system-uri-objects-instead-of-strings.md) | Passar objetos de uri em vez de cadeias de caracteres de sistema
...
CA2244 | Não duplica as inicializações do elemento indexado
...
CA3061 | Não adicione o esquema de URL
...
[CA3077](ca3077-insecure-processing-in-api-design-xml-document-and-xml-text-reader.md) | Processamento inseguro no Design de API, XmlDocument e XmlTextReader
[CA3147](ca3147-mark-verb-handlers-with-validateantiforgerytoken.md) | Manipuladores de verbo de marca com validar o Token Antifalsificação
[CA5350](ca5350-do-not-use-weak-cryptographic-algorithms.md) | Não usar algoritmos de criptografia fracos
[CA5351](ca5351-do-not-use-broken-cryptographic-algorithms.md) | Não Use algoritmos de criptografia
CA5358 | Não Use os modos de codificação não seguro
CA5359 | Não desabilite a validação de certificado
CA5360 | Não chame métodos perigosos na desserialização
CA5361 | Não desabilite o uso de SChannel de uma criptografia forte
CA5362 | Não faça referência Self na classe serializável
CA5363 | Não desabilite a validação de solicitação
CA5364 | Não Use protocolos de segurança preterido
CA5365 | Não desabilite a verificação de cabeçalho de HTTP
CA5366 | Use o XmlReader para leitura do conjunto de dados Xml
CA5367 | Não serializar os tipos com campos de ponteiro
CA5368 | Definir ViewStateUserKey para Classes derivadas de página
CA5369 | Use o XmlReader para desserializar
CA5370 | Usar o XmlReader para validar o leitor
CA5371 | Use o XmlReader para leitura do esquema
CA5372 | Use o XmlReader para XPathDocument
CA5373 | Não use a função de derivação de chave obsoleta
CA5374 | Não Use XslTransform
CA5375 | Não Use a assinatura de acesso compartilhado de conta
CA5376 | Use SharedAccessProtocol HttpsOnly
CA5377 | Usar a política de acesso no nível de contêiner
CA5378 | Não desabilite ServicePointManagerSecurityProtocols
CA5379 | Não Use o algoritmo de derivação de chave fracos de função
CA9999 | Incompatibilidade de versão do analisador

> [!NOTE]
> CA1052 regras e CA1053 da implementação original do FxCop são combinados em analisadores FxCop uma única regra, CA1052.

## <a name="unported-rules"></a>Regras unported License

O conjunto de regras que ainda não foi movido para o [analisadores FxCop](install-fxcop-analyzers.md) consiste em regras que ainda não o fez, mas ainda [podem ser portados](#rules-that-may-be-ported)e aqueles que foram preteridos e [não será transferido](#deprecated-rules).

### <a name="rules-that-may-be-ported"></a>Regras que podem ser portadas

As seguintes regras de análise de código estático do FxCop ainda não tiverem sido implementadas como analisadores, mas ainda poderão ocorrer. Isso pode ser devido a uma bloqueio razão técnica ou simplesmente que a regra é a prioridade mais baixa. Para obter mais informações sobre o status de portabilidade de cada regra, clique no link na **problema de controle** coluna.

ID da regra | Problema de rastreamento
--- | ---
...
As seguintes regras de análise de código estático do FxCop foram preteridas e não ser implementadas como analisadores. Para obter mais informações, você pode pesquisar por ID de regra (por exemplo, **CA1009**) sobre o [página de problemas do GitHub de analisadores de roslyn](https://github.com/dotnet/roslyn-analyzers/issues?utf8=%E2%9C%93&q=is:issue+label:FxCop-Port).
...
- [Regras de fxcopanalyzers](https://github.com/dotnet/roslyn-analyzers/blob/master/src/Microsoft.CodeAnalysis.FxCopAnalyzers/Microsoft.CodeAnalysis.FxCopAnalyzers.md)