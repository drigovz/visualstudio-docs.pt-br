...
Em vez disso, gere um tipo mais derivado que já existe na estrutura ou crie seu próprio tipo que deriva de <xref:System.Exception>.
...
|Valor de `enum` inválido|<xref:System.ComponentModel.InvalidEnumArgumentException?displayProperty=fullName>|
...
Quando uma operação não é suportada (como em um **fluxo substituído. gravar** em um fluxo aberto para leitura) throw <xref:System.NotSupportedException?displayProperty=fullName>

Quando uma conversão resultaria em um estouro (como em uma sobrecarga de operador de conversão explícita), lançaremos <xref:System.OverflowException?displayProperty=fullName>

Para todas as outras situações, considere a criação de seu próprio tipo que deriva de <xref:System.Exception> e lance isso.
