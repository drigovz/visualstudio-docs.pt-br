...
Um elemento de código que é marcado com o atributo <xref:System.Security.SecurityCriticalAttribute> é crítico para segurança. Um método transparente não pode usar um elemento crítico para segurança. Se um tipo transparente tentar usar um tipo crítico de segurança, um <xref:System.TypeAccessException>, <xref:System.MethodAccessException> ou <xref:System.FieldAccessException> será gerado.
