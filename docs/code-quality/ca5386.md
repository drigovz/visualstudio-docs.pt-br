...
- Um <xref:System.Net.SecurityProtocolType?displayProperty=nameWithType> valor seguro, mas codificado, foi referenciado.
- Um valor inteiro que representa uma versão de protocolo seguro foi atribuído <xref:System.Net.SecurityProtocolType> a uma variável.
