...
Um tipo implementa a interface <xref:System.Collections.IEnumerable?displayProperty=fullName>, mas não implementa a interface <xref:System.Collections.Generic.IEnumerable%601?displayProperty=fullName> e o assembly que a contém tem como destino .NET. Essa regra ignora os tipos que implementam <xref:System.Collections.IDictionary?displayProperty=fullName>.
...
O exemplo a seguir mostra uma classe (tipo de referência) que deriva da classe `CollectionBase` não genérica, que viola essa regra.
...
O exemplo a seguir corrige a violação alterando a classe base da coleção da classe de `CollectionBase` não genérica para a classe `Collection<T>` genérica (`Collection(Of T)` na Visual Basic).
...
O exemplo a seguir corrige a violação implementando essas interfaces genéricas: `IEnumerable<T>`, `ICollection<T>`e `IList<T>` (`IEnumerable(Of T)`, `ICollection(Of T)`e `IList(Of T)` no Visual Basic).
