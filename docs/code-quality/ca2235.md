...
Apenas suprimir um aviso dessa regra se um tipo de <xref:System.Runtime.Serialization.ISerializationSurrogate?displayProperty=fullName> for declarado que permite que instâncias do campo sejam serializadas e desserializadas.
...
A regra CA2235 não analisa os tipos que implementam a interface <xref:System.Runtime.Serialization.ISerializable> (a menos que também sejam marcadas com o atributo <xref:System.SerializableAttribute>). Isso ocorre porque a [regra CA2237](../code-quality/ca2237.md) já recomenda tipos de marcação que implementam a interface <xref:System.Runtime.Serialization.ISerializable> com o atributo <xref:System.SerializableAttribute>.
