...
## <a name="example"></a>Exemplo
O aplicativo a seguir usa a biblioteca que foi definida anteriormente. Observe que o tipo declarado fora de um namespace é criado quando o nome `Test` não é qualificado por um namespace. Observe também que, para acessar o tipo de `Test` no `Goodspace`, o nome do namespace é necessário.
