...
Por padrão, cada relação incorporada tem `PropagatesDelete` true na função de destino. Excluir `henry` não exclui o `familyTree`, mas `familyTree.Delete()` excluiria todas as `Persons`.
...
|Diagrama|<xref:Microsoft.VisualStudio.Modeling.Diagrams.Diagram>|
 ...
 As formas podem ter formas filhas em dois conjuntos. Uma forma no conjunto de `NestedChildShapes` é confinada à caixa delimitadora de seu pai. Uma forma na lista de `RelativeChildShapes` pode aparecer fora do limite ou parcialmente fora dos limites do pai, por exemplo, um rótulo ou uma porta. Um diagrama não tem nenhum `RelativeChildShapes` e nenhum `Parent`.
