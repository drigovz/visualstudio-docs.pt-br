...
Por padrão, cada relação incorporada tem `PropagatesDelete` true na função de destino. Excluir não exclui o `familyTree`, mas `familyTree.Delete()` exclui todos os `Persons`. `henry`
