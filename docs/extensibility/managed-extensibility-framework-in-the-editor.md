...
## <a name="overview-of-the-managed-extensibility-framework"></a>Visão geral do Managed Extensibility Framework
 ...
 O MEF está contido no assembly *System. ComponentModel. composição. dll* .NET Framework versão 4.
...
### <a name="export-and-import-component-parts"></a>Exportar e importar partes de componentes
 Você pode exportar qualquer funcionalidade, desde que ela seja implementada como uma classe pública ou um membro público de uma classe (Propriedade ou método). Você não precisa derivar a parte do componente de <xref:System.ComponentModel.Composition.Primitives.ComposablePart>. Em vez disso, você deve adicionar um atributo <xref:System.ComponentModel.Composition.ExportAttribute> à classe ou membro de classe que você deseja exportar. Esse atributo especifica o *contrato* pelo qual outra parte de componente pode importar sua funcionalidade.
...
```
 ...
 Por padrão, o atributo <xref:System.ComponentModel.Composition.ExportAttribute> define um contrato que é o tipo da classe de exportação.
...
```
 ...
 No exemplo, o atributo de `[Export]` padrão é equivalente a `[Export(typeof(TestAdornmentLayerDefinition))]`.
...
### <a name="import-a-mef-export"></a>Importar uma exportação do MEF
 Quando desejar consumir uma exportação de MEF, você deve saber o contrato (normalmente o tipo) pelo qual ele foi exportado e adicionar um atributo de <xref:System.ComponentModel.Composition.ImportAttribute> que tem esse valor. Por padrão, o atributo de importação usa um parâmetro, que é o tipo da classe que ele modifica. As linhas de código a seguir importam o tipo de <xref:Microsoft.VisualStudio.Text.Classification.IClassificationTypeRegistryService>.
...
1. Adicione referências a *System. composição. ComponentModel. dll*, que está no GAC (cache de assembly global) e aos assemblies do editor.
...
3. Adicione o atributo `[Import]` à sua interface de serviço, da seguinte maneira.
...
5. Depois de compilar o assembly, coloque-o no *.. \Common7\IDE\Components \* pasta de sua instalação do Visual Studio.
