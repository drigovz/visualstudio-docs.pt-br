...
#### <a name="use-win32-and-wpf-versions-of-ivstextview"></a>Usar versões de IVsTextView Win32 e WPF
 ...
 Para definir uma exibição de texto para o modo do WPF, você deve chamar <xref:Microsoft.VisualStudio.TextManager.Interop.IVsTextView.Initialize%2A> e passe [TextViewInitFlags3.VIF_NO_HWND_SUPPORT](<xref:Microsoft.VisualStudio.TextManager.Interop.TextViewInitFlags3.VIF_NO_HWND_SUPPORT>) conforme um da inicialização sinalizadores no `InitView` parâmetro. Você pode obter o <xref:System.Windows.FrameworkElement> chamando <xref:Microsoft.VisualStudio.Shell.Interop.IVsUIElementPane.CreateUIElementPane%2A>.
