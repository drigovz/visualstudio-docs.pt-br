...
## <a name="find-a-specific-service"></a>Localizar um serviço específico
 Você também pode usar o método <xref:Microsoft.VisualStudio.Settings.SettingsStore.CollectionExists%2A> para determinar se um serviço específico está instalado. Você deve saber o tipo da classe de serviço.

1. No MenuItemCallback do projeto que você criou no procedimento anterior, pesquise o repositório de definições de configuração para a coleção de `Services` que tem a subcoleção nomeada pelo GUID do serviço. Nesse caso, procuraremos o serviço de ajuda.
