...
Todos os projetos no Visual Studio devem implementar <xref:Microsoft.VisualStudio.Shell.Interop.IVsProject3> é reconhecido como um projeto dentro do IDE do Visual Studio. No entanto, essa interface não é especializada suficiente para o controle de origem. Projetos que devem estar na origem de controle de implementar <xref:Microsoft.VisualStudio.Shell.Interop.IVsSccProject2>. Essa interface é usada pelo VSPackage de controle de origem para consultar um projeto para seu conteúdo e fornecê-la glifos e informações de associação (as informações necessárias para estabelecer uma conexão entre o local do servidor e o local de disco de um projeto que está sob controle de origem).

O controle de origem implementa VSPackage <xref:Microsoft.VisualStudio.Shell.Interop.IVsSccManager2>, que por sua vez permite que os projetos se registrem para controle do código-fonte e recuperar os glifos de status.
