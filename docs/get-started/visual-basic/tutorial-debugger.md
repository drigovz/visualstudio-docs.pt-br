...
Embora o aplicativo de demonstração seja Visual Basic, a maioria dos recursos é aplicável C#a C++, F#,, Python, JavaScript e a outras linguagens com suporte noF# Visual Studio (não oferece suporte a editar e continuar. F# e JavaScript não dão suporte à janela **Autos**). As capturas de tela estão em Visual Basic.
...
1. Enquanto estiver em pausa na instrução `name += letters[i]`, focalize a variável `letters` e você verá o valor padrão, o valor do primeiro elemento na matriz, `"f"c`.
