---
title: 'Tutorial: Depurar código visual básico'
...
Embora o aplicativo de demonstração seja visual básico, a maioria dos recursos são aplicáveis a C#, C++, F#, Python, JavaScript e outras linguagens suportadas pelo Visual Studio (F# não suporta Edição e continua. F# e JavaScript não dão suporte à janela **Autos**). As capturas de tela estão no Visual Basic.
...
3. Na caixa de diálogo **Novo Projeto**, no painel esquerdo, expanda **Visual Basic** e, em seguida, escolha **.NET Core**. No painel central, escolha **Aplicativo de Console (.NET Core)**. Em seguida, nomeie o projeto *get-started-debugging*.
...
1. Na janela **Criar um novo projeto**, insira ou digite *console* na caixa de pesquisa. Em seguida, escolha **Visual Basic** na lista Linguagem de programação e, em seguida, escolha **Windows** na lista Plataforma. 
   ...
   ![Escolha o modelo Visual Basic para o aplicativo de console (.NET Core)](../visual-basic/media/vs-2019/get-started-create-console-project.png)
...
1. Pressione **F11** mais uma `SendMessage` vez para entrar no método.
     ...
     ![Use f11 para entrar no código](../visual-basic/media/get-started-f11-vb.png "F10 Passo Em")
...
1. Durante uma pausa na chamada do método, **pressione F10** (ou escolha **Depurar > Step Over**) uma vez.
     ...
     ![Use f10 para passar por cima do código](../visual-basic/media/get-started-step-over-vb.png "F10 Step Over")
...
1. No editor de código, role para `Console.WriteLine` baixo `SendMessage` e passe o mouse sobre o método no método até que o botão **Executar para clicar** em Executar para ![clicar](../../debugger/media/dbg-tour-run-to-click.png "RunToClick") apareça à esquerda. A dica de ferramenta para o botão mostra "Realizar a execução até aqui".
     ...
     ![Use o recurso Executar para clicar](../visual-basic/media/get-started-run-to-click-vb.png "Executar com um Clique")
...
1. Enquanto pausado `name += letters[i]` na declaração, `letters` paire sobre a variável e você vê seu valor `"f"c`padrão, o valor do primeiro elemento na matriz, .
...
1. Pressione **F5** (ou **Debug** > **Continue)** algumas vezes para `For` iterar várias vezes através do `name` loop, parando novamente no ponto de quebra e pairando sobre a variável cada vez para verificar seu valor.
     ...
     ![Ver uma dica de dados](../visual-basic/media/get-started-data-tip-vb.png "Exibir uma dica de dados")
...
1. Expanda `letters` a variável para mostrar os elementos que ela contém.
     ...
     ![Inspecione variáveis na janela Locais](../visual-basic/media/get-started-locals-window-vb.png "Janela Locais")
