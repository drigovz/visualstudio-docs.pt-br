...
Para depurar um processo do .NET Core em um contêiner do Docker do Linux, consulte [anexar a um contêiner do Docker do Linux](#BKMK_Docker_Attach).
::: moniker-end
...
1. No Visual Studio, selecione **depurar** > **anexar ao processo** (ou pressione **Ctrl**+**ALT**+**P**) para abrir a caixa de diálogo **anexar ao processo** .
   ...
   O **tipo de conexão** deve ser definido como **padrão**. O **destino da conexão** deve ser o nome do computador local.

   ![DBG_Basics_Attach_To_Process](../debugger/media/DBG_Basics_Attach_To_Process.png "DBG_Basics_Attach_To_Process")

2. Na lista **processos disponíveis** , localize e selecione o processo ou os processos aos quais você deseja anexar.
...
3. No campo **anexar a** , verifique se o tipo de código que você planeja Depurar está listado. A configuração **automática** padrão funciona para a maioria dos tipos de aplicativos.
...
2. O **tipo de conexão** deve ser o **padrão** para a maioria dos casos. Na caixa **destino da conexão** , selecione o computador remoto, usando um dos seguintes métodos:
...
## <a name="BKMK_Docker_Attach"></a>Anexar a um processo em execução em um contêiner do Docker do Linux
...
4. Escolha o processo de contêiner correspondente na lista de **processos disponíveis** e selecione **anexar** para iniciar a depuração C# do processo de contêiner no Visual Studio!

    ![Menu de anexo do Docker concluído](../debugger/media/docker-attach-complete.png "Menu de anexo do Docker concluído")


::: moniker-end

## <a name="BKMK_reattach"></a>Reanexar a um processo

Você pode reanexar rapidamente aos processos aos quais você foi anexado anteriormente, escolhendo **depurar** > **reanexar para processar** (**Shift**+**ALT**+**P**). Quando você escolher esse comando, o depurador tentará imediatamente se anexar aos últimos processos anexados pela primeira vez tentando corresponder à ID do processo anterior e, se isso falhar, correspondendo ao nome do processo anterior. Se nenhuma correspondência for encontrada ou se vários processos tiverem o mesmo nome, a caixa de diálogo **anexar ao processo** será aberta para que você possa selecionar o processo correto.
...
|Depurar um aplicativo universal do Windows (UWP), OneCore, HoloLens ou aplicativo IoT que você não iniciou no Visual Studio|Depurar pacote do aplicativo instalado|{1&gt;N/A&lt;1}|Confira [depurar um pacote do aplicativo instalado](debug-installed-app-package.md) em vez de usar **anexar ao processo**|
