...
**Usar o novo Auxiliar de Exceção**:  
...
- **Função de conversão de cadeia de caracteres de chamada em objetos em janelas de variáveis (somente C# e JavaScript)**:  
...
- **Permite ao servidor de origem confiar parcialmente em assemblies (somente gerenciado)**:  
...
- **Sempre executar comandos não confiáveis do servidor de código-fonte sem perguntar**:  
...
**Habilitar suporte de Link de Origem**:  
...
- **Fazer o fallback para autenticação de Gerenciador de Credenciais do Git para todas as solicitações de Vínculo da Fonte**:  
...
**Redirecionar todo o texto da Janela Saída para a Janela Imediato**:  
...
**Habilitar depuração do JavaScript para ASP.NET (Chrome, Microsoft Edge e IE)**:  
...
**Habilitar as Ferramentas para Desenvolvedores do Microsoft Edge para Aplicativos de JavaScript da UWP (Experimental)**:  
...
**Usar maneira experimental de iniciar a depuração de JavaScript do Chrome ao executar o Visual Studio como Administrador**:  
...
**Usar os avaliadores da expressão de C# e VB herdados**:  
...
**Avisar ao usar visualizadores do depurador personalizados contra processos potencialmente inseguros (somente Gerenciados)**:  
...
**Habilitar as Ferramentas de Depuração da Interface do Usuário para XAML**:  
...
**Fechar automaticamente o console quando a depuração parar**:  
...
**Voltar para a pilha de chamadas em exceções não tratadas**:  
