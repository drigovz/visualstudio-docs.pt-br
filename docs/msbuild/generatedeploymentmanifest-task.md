...
description: Aprenda a usar a tarefa MSBuild GenerateDeploymentManifest para gerar um manifesto de implantação do ClickOnce.
...
| `MapFileExtensions` | Parâmetro `Boolean` opcional.<br /><br /> Especifica se o mapeamento de extensão de nome de arquivo *. Deploy* é usado. Se esse parâmetro for `true` , todos os arquivos de programa serão publicados com uma extensão de nome de arquivo *. Deploy* . Essa opção é útil para a segurança do servidor Web limitar o número de extensões de nome de arquivo que devem ser desbloqueadas para habilitar a implantação do aplicativo ClickOnce. O valor padrão desse parâmetro é `false`. |
...
| `SupportUrl` | Parâmetro `String` opcional.<br /><br /> Especifica o link que aparece na caixa de diálogo **Adicionar ou remover programas** do aplicativo. O valor especificado deve ser um caminho de URL ou UNC totalmente qualificado. |
