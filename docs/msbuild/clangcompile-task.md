...
|**BufferSecurityCheck**|Parâmetro **string** opcional.<br/><br/>A Verificação de Segurança ajuda a detectar saturações de buffer de pilha, uma tentativa de ataque comum à segurança de um programa. <br/><br/>Use `fstack-protector`.|
|**BuildingInIde**|Parâmetro opcional **bool**.|
|**CLanguageStandard**|Parâmetro **string** opcional.<br/><br/>Determina o padrão de linguagem C.<br/><br/>Use `std=[value]` com o valor de **c89**, **c99**, **c11**, **gnu99** ou **gnu11**.|
|**ClangVersion**|Parâmetro **string** opcional.|
|**CompileAs**|Parâmetro **string** opcional.<br/><br/>Selecione a opção de linguagem de compilação para arquivos .c e .cpp. Padrão detectará com base na extensão .c ou .cpp.<br/><br/>Use `-x c`, `-x c++`.|
|**CppLanguageStandard**|Parâmetro **string** opcional.<br/><br/>Determina o padrão de linguagem C++.<br/><br/>Use `std=[value]` com o valor de **c++98**, **c++11**, **c++1y**, **gnu++98**, **gnu++11** ou **gnu++1y**.|
...
|**DebugInformationFormat**|Parâmetro **string** opcional.<br/><br/>Especifica o tipo de informações de depuração geradas pelo compilador.<br/><br/>**Nenhum**, não produz informação de depuração; portanto, a compilação pode ser mais rápida (use `g0`).<br/>**FullDebug**, gere informações de depuração DWARF2 (use `g2 -gdwarf-2`).<br/>**LineNumber**, gere somente informações de Número de Linha (use `gline-tables-only`).|
...
|**GccToolChain**|Parâmetro **string** opcional.<br/><br/>Caminho da pasta para Cadeia de Ferramenta Gcc.|
|**GNUMode**|Parâmetro opcional **bool**.<br/><br/>|
|**MSCompatibility**|Parâmetro opcional **bool**.<br/><br/>Habilite a C++ compatibilidade total da Microsoft.|
|**MSCompatibilityVersion**|Parâmetro **string** opcional.<br/><br/>O valor separado por ponto que representa o número de versão do compilador da Microsoft a relatar em _MSC_VER (0 = não definir (padrão)).|
...
|**MSCompilerVersion**|Parâmetro **string** opcional.<br/><br/>O número de versão do compilador da Microsoft para relatar no _MSC_VER (0 = não definir (padrão)).|
...
|**PrecompiledHeader**|Parâmetro **string** opcional.<br/><br/>Habilita a criação ou o uso de um cabeçalho pré-compilado durante o build.|
...
|**ThumbMode**|Parâmetro **string** opcional.<br/><br/>Gerar um código que pode ser executado em microarquiteturas thumb. Somente se aplica a arquiteturas arm.<br/><br/>**Thumb**, gere código Thumb (use `mthumb`).<br/>**ARM**, gere código Arm (use `marm`).<br/>**Desabilitado**, opção não aplicável à plataforma escolhida.|
|**TrackerLogDirectory**|Parâmetro **string** opcional.<br/><br/>Diretório de Log do Rastreador.|
