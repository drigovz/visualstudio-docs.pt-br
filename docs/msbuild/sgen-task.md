...
Cria um assembly de serialização de XML para tipos no assembly especificado. Esta tarefa envolve a ferramenta Gerador serializador XML *(Sgen.exe).* Para saber mais, confira [Ferramenta geradora de serializador de XML (Sgen.exe)](/dotnet/framework/serialization/xml-serializer-generator-tool-sgen-exe).
...
| `ToolPath` | Parâmetro `String` opcional.<br /><br /> Especifica o local de onde a tarefa carregará o arquivo executável subjacente *(sgen.exe).* Se esse parâmetro não for especificado, a tarefa usará o caminho de instalação do SDK correspondente à versão da estrutura que está executando o MSBuild. |
