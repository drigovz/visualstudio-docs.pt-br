...
description: O IntelliTest explora seu código .NET para gerar dados de teste e um pacote de testes de unidade. Saiba como executar o IntelliTest para ver quais testes estão falhando e corrigi-los.
...
1. Selecione as linhas de dados que deseja salvar com o teste de unidade parametrizado em um projeto de teste.
     ...
     Você pode exibir o projeto de teste e o teste de unidade com parâmetros que foram criados – os testes de unidade individuais, correspondentes a cada uma das linhas, são salvos no arquivo *. g.cs* no projeto de teste, e um teste de unidade com parâmetros é salvo em seu arquivo *. cs* correspondente. É possível executar os testes de unidade e exibir os resultados do Gerenciador de Testes, como você faria com qualquer teste de unidade criado manualmente.
...
**R:** Sim, use pressuposições para especificar quais dados de teste não são necessários para o teste de unidade para um método específico. Use a classe <xref:Microsoft.Pex.Framework.PexAssume> para adicionar suposições. Por exemplo, você pode adicionar uma pressuposição de que a variável `lengths` não é nula, como a seguir:
