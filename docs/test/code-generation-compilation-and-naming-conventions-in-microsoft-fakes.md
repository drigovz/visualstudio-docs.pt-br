---
title: 'Microsoft Fakes: Gerar código de compilação &; convenções de nomeação'
ms.date: 11/04/2016
ms.topic: conceptual
ms.author: mikejo
manager: jillfra
...
Você pode especificar uma chave pública diferente para o conjunto Fakes, como uma chave criada para o conjunto shimmed, especificando o caminho `Fakes` \\ `Compilation` completo para o arquivo *.snk* que contém a chave alternativa como o valor de atributo `KeyFile` no elemento do arquivo *.fakes.* Por exemplo: 
...
- Se o método for genérico, `Of`*n* será acrescentado, em que *n* é o número de argumentos de método genérico.
  ...
  **Nomes de métodos especiais,** como o getter de propriedades ou setters são tratados conforme descrito na tabela a seguir:
...
|Um **tipo genérico** `!i` de argumento de tipo`C<TType>`|`Ti`|
|Um **argumento** `!!i` de método genérico`M<MMethod>`|`Mi`|
...
- [Código isolante em teste com o Microsoft Fakes](../test/isolating-code-under-test-with-microsoft-fakes.md)
