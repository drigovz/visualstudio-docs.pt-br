...
## <a name="understand-worksheet-host-items-in-document-level-projects"></a>Entender os itens de host da planilha em projetos de nível de documento
 Quando você cria um projeto de nível de documento para o Excel, o Visual Studio <xref:Microsoft.Office.Tools.Excel.Worksheet> cria automaticamente três itens de host no projeto. Os nomes padrão das planilhas são `Sheet1`, `Sheet2`e `Sheet3`. Se você criar um projeto baseado em uma pasta de trabalho existente, o número de itens do host dependerá do número de planilhas na pasta de trabalho.
...
### <a name="limitations-of-the-worksheet-host-item-in-document-level-projects"></a>Limitações do item de host de planilha em projetos de nível de documento
 Você não pode criar <xref:Microsoft.Office.Tools.Excel.Worksheet> novos itens de host em tempo de execução em um projeto de nível de documento. Se você criar uma nova planilha do Excel em tempo de execução, ela será do tipo <xref:Microsoft.Office.Interop.Excel.Worksheet>. Como não é um item de host, ele não pode conter controles de host nem controles de Windows Forms. Para obter mais informações sobre como criar documentos em tempo de [execução, consulte Como: Adicione programaticamente novas planilhas a](../vsto/how-to-programmatically-add-new-worksheets-to-workbooks.md)pastas de trabalho.
...
## <a name="understand-worksheet-host-items-in-vsto-add-in-projects"></a>Entender os itens de host da planilha nos projetos do suplemento do VSTO
 Em projetos de nível de aplicativo, você pode gerar <xref:Microsoft.Office.Tools.Excel.Worksheet> um item de host em tempo de execução para qualquer planilha aberta no Excel. Você pode usar o <xref:Microsoft.Office.Tools.Excel.Worksheet> item de host para adicionar controles à planilha associada ou para manipular eventos que não estão disponíveis em <xref:Microsoft.Office.Interop.Excel.Worksheet> objetos.

 Para gerar um <xref:Microsoft.Office.Tools.Excel.Worksheet> item de host, use `GetVstoObject` o método. Para obter mais informações, consulte [estender documentos do Word e pastas de trabalho do Excel em suplementos do VSTO em tempo de execução](../vsto/extending-word-documents-and-excel-workbooks-in-vsto-add-ins-at-run-time.md).
