...
- Em projetos de nível de documento para o Word, a classe `ThisDocument` é chamada por padrão.
...
- Projetos de nível de documento para Excel têm várias classes geradas: uma para a própria pasta de trabalho e uma para cada planilha. Por padrão, essas classes têm os seguintes nomes:
  ...
  A classe gerada inclui manipuladores de eventos que são chamados quando o documento é aberto ou fechado. Para executar o código quando o documento é aberto, adicione o código `Startup` ao manipulador de eventos. Para executar o código logo antes de o documento ser fechado, adicione o `Shutdown` código ao manipulador de eventos. Para obter mais informações, consulte [eventos em projetos do Office](../vsto/events-in-office-projects.md).

### <a name="understand-the-design-of-the-generated-classes"></a>Entender o design das classes geradas
 Em projetos que visam [!INCLUDE[net_v40_short](../sharepoint/includes/net-v40-short-md.md)] o ou [!INCLUDE[net_v45](../vsto/includes/net-v45-md.md)]o, os tipos de item de [!INCLUDE[vsto_runtime](../vsto/includes/vsto-runtime-md.md)] host no são interfaces, portanto, as classes geradas não podem derivar sua implementação deles. Em vez disso, as classes geradas derivam a maioria de seus membros das seguintes classes base:
...
- `Sheet`*n*: deriva de <xref:Microsoft.Office.Tools.Excel.WorksheetBase>.
  ...
  Essas classes base redirecionam todas as chamadas para seus membros para implementações internas das interfaces de item [!INCLUDE[vsto_runtime](../vsto/includes/vsto-runtime-md.md)]de host correspondentes no. Por exemplo, se você chamar o <xref:Microsoft.Office.Tools.Word.DocumentBase.Protect%2A> método `ThisDocument` da classe, a <xref:Microsoft.Office.Tools.Word.DocumentBase> classe redirecionará <xref:Microsoft.Office.Tools.Word.Document> essa chamada para [!INCLUDE[vsto_runtime](../vsto/includes/vsto-runtime-md.md)]a implementação interna da interface no.

## <a name="access-the-object-model-of-the-host-application"></a>Acessar o modelo de objeto do aplicativo host
 Para acessar o modelo de objeto do aplicativo host, use membros da classe gerada em seu projeto. Cada uma dessas classes corresponde a um objeto no modelo de objeto do Excel ou Word e contém a maioria das mesmas propriedades, métodos e eventos. Por exemplo, a `ThisDocument` classe em um projeto de nível de documento do Word fornece a maioria dos mesmos membros que <xref:Microsoft.Office.Interop.Word.Document> o objeto no modelo de objeto do Word.

 O exemplo de código a seguir mostra como usar o modelo de objeto do Word para salvar o documento que faz parte de uma personalização em nível de documento para o Word. Este exemplo deve ser executado a partir `ThisDocument` da classe.
...
```
 ...
 Como a `ThisDocument` classe obtém a maioria de seus membros <xref:Microsoft.Office.Tools.Word.Document> do item de host, o `Save` método que é chamado nesse código é, na verdade <xref:Microsoft.Office.Tools.Word.Document.Save%2A> , o método <xref:Microsoft.Office.Tools.Word.Document> do item de host. Esse método corresponde ao <xref:Microsoft.Office.Interop.Word._Document.Save%2A> método <xref:Microsoft.Office.Interop.Word.Document> do objeto no modelo de objeto do Word.
 ...
 Para obter mais informações sobre `Globals` o objeto, consulte [acesso global a objetos em projetos do Office](../vsto/global-access-to-objects-in-office-projects.md).
...
## <a name="add-controls-to-documents"></a>Adicionar controles a documentos
 ...
 Os controles de host são classes que estendem alguns dos objetos no modelo de objeto do Word e do Excel. Por exemplo, o <xref:Microsoft.Office.Tools.Excel.ListObject> controle de host fornece toda a funcionalidade <xref:Microsoft.Office.Interop.Excel.ListObject> do no Excel. No entanto <xref:Microsoft.Office.Tools.Excel.ListObject> , o controle de host também tem eventos adicionais e recursos de associação de dados.
...
- Adicione grupos personalizados a uma guia interna na faixa de bits.
   ...
   Para obter mais informações, confira [Como: Personalize uma guia](../vsto/how-to-customize-a-built-in-tab.md)interna.
