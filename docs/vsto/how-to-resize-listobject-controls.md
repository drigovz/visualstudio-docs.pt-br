...
ms.locfileid: "72985748"
---
# <a name="how-to-resize-listobject-controls"></a>Como: redimensionar controles de ListObject
  Você define o tamanho de um controle de <xref:Microsoft.Office.Tools.Excel.ListObject> ao adicioná-lo a uma pasta de trabalho do Microsoft Office Excel; no entanto, talvez você queira redimensioná-lo posteriormente. Por exemplo, talvez você queira alterar uma lista de duas colunas para três colunas.
...
- [Redimensionar controles ListObject em tempo de execução em um projeto de suplemento do VSTO](#runtimeaddin)
  ...
  Para obter mais informações sobre controles de <xref:Microsoft.Office.Tools.Excel.ListObject>, consulte o [controle ListObject](../vsto/listobject-control.md).
...
1. Clique em qualquer lugar na tabela <xref:Microsoft.Office.Tools.Excel.ListObject>. A guia **ferramentas de tabela** > **design** na faixa de faixas é exibida.
...
## <a name="runtimedoclevel"></a>Redimensionar um controle ListObject em tempo de execução em um projeto de nível de documento
 Você pode redimensionar um controle de <xref:Microsoft.Office.Tools.Excel.ListObject> em tempo de execução usando o método <xref:Microsoft.Office.Tools.Excel.ListObject.Resize%2A>. Você não pode usar esse método para mover o controle de <xref:Microsoft.Office.Tools.Excel.ListObject> para um novo local na planilha. Os cabeçalhos devem permanecer na mesma linha e o controle de <xref:Microsoft.Office.Tools.Excel.ListObject> redimensionado deve sobrepor o objeto de lista original. O controle de <xref:Microsoft.Office.Tools.Excel.ListObject> redimensionado deve conter uma linha de cabeçalho e pelo menos uma linha de dados.

### <a name="to-resize-a-list-object-programmatically"></a>Para redimensionar um objeto de lista programaticamente

1. Crie um controle de <xref:Microsoft.Office.Tools.Excel.ListObject> que abranja a célula **a1** a **B3** em `Sheet1`.
...
## <a name="runtimeaddin"></a>Redimensionar um ListObject em tempo de execução em um projeto de suplemento do VSTO
 Você pode redimensionar um controle de <xref:Microsoft.Office.Tools.Excel.ListObject> em qualquer planilha aberta em tempo de execução. Para obter mais informações sobre como adicionar um controle de <xref:Microsoft.Office.Tools.Excel.ListObject> a uma planilha usando um suplemento do VSTO, consulte [como: adicionar controles ListObject a planilhas](../vsto/how-to-add-listobject-controls-to-worksheets.md).
