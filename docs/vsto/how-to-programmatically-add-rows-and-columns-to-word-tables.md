...
# <a name="how-to-programmatically-add-rows-and-columns-to-word-tables"></a>Como: adicionar linhas e colunas programaticamente a tabelas do Word
  Em uma Microsoft Office tabela do Word, as células são organizadas em linhas e colunas. Você pode usar o método <xref:Microsoft.Office.Interop.Word.Rows.Add%2A> do objeto <xref:Microsoft.Office.Interop.Word.Rows> para adicionar linhas à tabela e o método <xref:Microsoft.Office.Interop.Word.Columns.Add%2A> do objeto <xref:Microsoft.Office.Interop.Word.Columns> para adicionar colunas.
...
## <a name="document-level-customization-examples"></a>Exemplos de personalização no nível do documento
 Os exemplos de código a seguir podem ser usados em uma personalização em nível de documento. Para usar esses exemplos, execute-os da classe `ThisDocument` em seu projeto. Esses exemplos pressupõem que o documento associado à sua personalização já tenha pelo menos uma tabela.
...
1. Use o método <xref:Microsoft.Office.Interop.Word.Rows.Add%2A> para adicionar uma linha à tabela.
...
1. Use o método <xref:Microsoft.Office.Interop.Word.Columns.Add%2A> e, em seguida, use o método <xref:Microsoft.Office.Interop.Word.Columns.DistributeWidth%2A> para fazer com que todas as colunas tenham a mesma largura.
...
## <a name="vsto-add-in-examples"></a>Exemplos de suplemento do VSTO
 Os exemplos de código a seguir podem ser usados em um suplemento do VSTO. Para usar os exemplos, execute-os da classe `ThisAddIn` em seu projeto. Esses exemplos pressupõem que o documento ativo já tenha pelo menos uma tabela.
