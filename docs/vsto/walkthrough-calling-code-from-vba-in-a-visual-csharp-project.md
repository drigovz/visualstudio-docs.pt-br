...
2. Adicione o código a seguir à classe `Sheet1`. O `CreateVstoNamedRange` método cria um novo <xref:Microsoft.Office.Tools.Excel.NamedRange> objeto no intervalo especificado. Esse método cria um manipulador de eventos para o <xref:Microsoft.Office.Tools.Excel.NamedRange.Selected> eventos do <xref:Microsoft.Office.Tools.Excel.NamedRange>. Posteriormente neste passo a passo, você chamará o `CreateVstoNamedRange` método do código do VBA no documento.
...
- Chame o código em um item de host em uma personalização no Visual Basic do VBA. Esse processo é diferente do processo Visual c#. Para obter mais informações, confira [Passo a passo: Chamar o código do VBA em um projeto do Visual Basic](../vsto/walkthrough-calling-code-from-vba-in-a-visual-basic-project.md).
