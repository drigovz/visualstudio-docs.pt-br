...
No Visual Studio, crie um projeto de **aplicativo Web ASP.NET Core** , `WebFrontEnd`chamado. Selecione **aplicativo Web** para criar um aplicativo Web com páginas Razor. 
...
Adicione um projeto à mesma solução e chame-o de *MyWebAPI*. Selecione **API** como o tipo de projeto e desmarque a caixa de seleção **Configurar para https**. Nesse design, estamos usando SSL apenas para comunicação com o cliente, não para comunicação entre contêineres no mesmo aplicativo Web. Só `WebFrontEnd` precisa de HTTPS.
...
1. No projeto, abra o arquivo *index.cshtml.cs* e substitua o `OnGet` método pelo código a seguir. `WebFrontEnd`
...
1. Agora, no projeto de API Web, adicione o código ao controlador de valores para personalizar a mensagem retornada pela API para a chamada que você adicionou do *WebFrontEnd*.
...
1. No projeto, escolha **Adicionar > contêiner de suporte do orquestrador.** `WebFrontEnd` A caixa de diálogo **Opções de suporte do Docker** é exibida.
...
1. Execute o site localmente agora (F5 ou CTRL + F5) para verificar se ele funciona conforme o esperado. Se tudo estiver configurado corretamente, você verá a mensagem "Olá, de WebFrontEnd e webAPI (com valor 1)".
   ...
   Veja o que você vê quando iniciado:
