...
Os TableAdapters estendem a funcionalidade de adaptadores de dados padrão encapsulando uma classe de <xref:System.Data.Common.DataAdapter> configurada. Por padrão, o TableAdapter herda da classe <xref:System.ComponentModel.Component> e não pode ser convertido na classe <xref:System.Data.Common.DataAdapter>. A conversão de um TableAdapter para a classe <xref:System.Data.Common.DataAdapter> resulta em um erro de <xref:System.InvalidCastException>. Para alterar a classe base de um TableAdapter, você pode especificar uma classe que deriva de <xref:System.ComponentModel.Component> na propriedade da **classe base** do TableAdapter no **Designer de conjunto de dados**.
...
Os TableAdapters oferecem suporte a tipos anuláveis `Nullable(Of T)` e `T?`. Para obter mais informações sobre tipos que permitem valor nulo no Visual Basic, consulte [Tipos de valores que permitem valor nulo](/dotnet/visual-basic/programming-guide/language-features/data-types/nullable-value-types). Para obter mais informações sobre tipos anuláveis no C#, consulte [usar tipos anuláveis](/dotnet/csharp/programming-guide/nullable-types/using-nullable-types).
