...
- A seleção da opção **criar métodos para enviar atualizações diretamente para o banco de dados** é equivalente a definir a propriedade `GenerateDBDirectMethods` como true. A opção fica não disponível quando a instrução SQL original não fornecer informações suficientes ou a consulta não for uma consulta atualizável. Essa situação pode ocorrer, por exemplo, em consultas de **junção** e consultas que retornam um único valor (escalar).
