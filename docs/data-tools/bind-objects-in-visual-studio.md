...
> [!NOTE]
> O Visual Studio nomeia as consultas `Fill` do `GetData` TableAdapter e, por padrão, mas você pode alterar esses nomes para qualquer nome de método válido.
...
O código a seguir mostra como criar a classe para uma coleção de `Order` objetos fortemente tipada:
...
Você adiciona objetos a uma coleção chamando o `Add` método da sua classe de coleção personalizada ou <xref:System.Windows.Forms.BindingSource>do.
...
O código a seguir mostra como adicionar objetos a uma coleção tipada que herda <xref:System.ComponentModel.BindingList%601>de:

> [!NOTE]
> Neste exemplo, a `Orders` coleção é uma propriedade `Customer` do objeto.
...
> [!NOTE]
> Os `Remove` métodos `RemoveAt` e são fornecidos automaticamente para sua coleção personalizada quando você herda de <xref:System.ComponentModel.BindingList%601>.
...
Se seu aplicativo exigir o acompanhamento de alterações e a reversão de alterações propostas para seus valores originais, você deverá implementar essa funcionalidade em seu modelo de objeto. Para obter exemplos de como as tabelas de dados acompanham as alterações <xref:System.Data.DataRowState>propostas <xref:System.Data.DataSet.HasChanges%2A>, consulte <xref:System.Data.DataTable.GetChanges%2A>, e.
