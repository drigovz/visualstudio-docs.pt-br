...
O melhor local para adicionar validação ao seu aplicativo é no arquivo de classe parcial do conjunto de um. No [!INCLUDE[vbprvb](../code-quality/includes/vbprvb_md.md)] ou[!INCLUDE[csprcs](../data-tools/includes/csprcs_md.md)], abra o **Designer de conjunto de dados** e clique duas vezes na coluna ou tabela para a qual você deseja criar a validação. Essa ação cria automaticamente um <xref:System.Data.DataTable.ColumnChanging> manipulador <xref:System.Data.DataTable.RowChanging> de eventos ou.

## <a name="validate-data"></a>Validar dados
A validação dentro de um conjunto de uma é feita das seguintes maneiras:

- Criando sua própria validação específica do aplicativo que pode verificar valores em uma coluna de dados individual durante as alterações. Para obter mais informações, confira [Como: Validar dados durante alterações](validate-data-in-datasets.md)de coluna.

- Criando sua própria validação específica do aplicativo que pode verificar os dados em valores enquanto uma linha de dados inteira está sendo alterada. Para obter mais informações, confira [Como: Validar os dados durante as](validate-data-in-datasets.md)alterações de linha.

- Criando chaves, restrições exclusivas e assim por diante como parte da definição de esquema real do conjunto de dados.

- Definindo <xref:System.Data.DataColumn> as propriedades do objeto, <xref:System.Data.DataColumn.MaxLength%2A>como, <xref:System.Data.DataColumn.AllowDBNull%2A>e <xref:System.Data.DataColumn.Unique%2A>.

Vários eventos são gerados pelo <xref:System.Data.DataTable> objeto quando uma alteração está ocorrendo em um registro:

- Os <xref:System.Data.DataTable.ColumnChanging> eventos <xref:System.Data.DataTable.ColumnChanged> e são gerados durante e após cada alteração em uma coluna individual. O <xref:System.Data.DataTable.ColumnChanging> evento é útil quando você deseja validar as alterações em colunas específicas. As informações sobre a alteração proposta são passadas como um argumento com o evento.
- Os <xref:System.Data.DataTable.RowChanging> eventos <xref:System.Data.DataTable.RowChanged> e são gerados durante e após qualquer alteração em uma linha. O <xref:System.Data.DataTable.RowChanging> evento é mais geral. Isso indica que uma alteração está ocorrendo em algum lugar na linha, mas você não sabe qual coluna foi alterada.

Por padrão, cada alteração em uma coluna, portanto, gera quatro eventos. O primeiro é o <xref:System.Data.DataTable.ColumnChanging> e <xref:System.Data.DataTable.ColumnChanged> os eventos para a coluna específica que está sendo alterada. Em seguida, <xref:System.Data.DataTable.RowChanging> estão <xref:System.Data.DataTable.RowChanged> os eventos e. Se várias alterações estiverem sendo feitas na linha, os eventos serão gerados para cada alteração.

> [!NOTE]
> O método da <xref:System.Data.DataRow.BeginEdit%2A> linha de dados desativa os eventos <xref:System.Data.DataTable.RowChanged> e após cada alteração de <xref:System.Data.DataTable.RowChanging> coluna individual. Nesse caso, o evento não é gerado até que o <xref:System.Data.DataRow.EndEdit%2A> método seja chamado, quando os <xref:System.Data.DataTable.RowChanging> eventos e <xref:System.Data.DataTable.RowChanged> são gerados apenas uma vez. Para obter mais informações, consulte [desativar restrições ao preencher um conjunto de](../data-tools/turn-off-constraints-while-filling-a-dataset.md)dados.

O evento escolhido depende de quão granular você deseja que a validação seja. Se for importante que você capture um erro imediatamente quando uma coluna for alterada, crie a validação usando o <xref:System.Data.DataTable.ColumnChanging> evento. Caso contrário, use <xref:System.Data.DataTable.RowChanging> o evento, o que pode resultar na captura de vários erros ao mesmo tempo. Além disso, se os dados forem estruturados para que o valor de uma coluna seja validado com base no conteúdo de outra coluna, execute sua <xref:System.Data.DataTable.RowChanging> validação durante o evento.

Quando os registros são atualizados, <xref:System.Data.DataTable> o objeto gera eventos que você pode responder à medida que as alterações estão ocorrendo e depois que as alterações são feitas.

Se seu aplicativo usar um dataset tipado, você poderá criar manipuladores de eventos com rigidez de tipos. Isso adiciona quatro eventos tipados adicionais para os quais você pode criar manipuladores `dataTableNameRowChanged`: `dataTableNameRowDeleting` `dataTableNameRowChanging`,, `dataTableNameRowDeleted`e. Esses manipuladores de eventos tipados passam um argumento que inclui os nomes de coluna da tabela que facilitam a gravação e a leitura do código.
...
Os <xref:System.Data.DataTable.ColumnChanging>eventos <xref:System.Data.DataTable.RowChanging>, e<xref:System.Data.DataTable.RowDeleting> são gerados durante o processo de atualização. Você pode usar esses eventos para validar dados ou executar outros tipos de processamento. Como a atualização está em andamento durante esses eventos, você pode cancelá-la lançando uma exceção, o que impede a conclusão da atualização.

Os <xref:System.Data.DataTable.ColumnChanged>eventos e<xref:System.Data.DataTable.RowDeleted>são eventos de notificação que são gerados quando a atualização é concluída com êxito. <xref:System.Data.DataTable.RowChanged> Esses eventos são úteis quando você deseja realizar uma ação adicional com base em uma atualização bem-sucedida.
...
- Opcionalmente, use <xref:System.Windows.Forms.ErrorProvider> um controle para exibir uma mensagem de erro para o usuário. Para obter mais informações, confira [Componente ErrorProvider](/dotnet/framework/winforms/controls/errorprovider-component-windows-forms).

A validação também pode ser executada durante <xref:System.Data.DataTable.RowChanging> o evento.

## <a name="validate-data-during-row-changes"></a>Validar dados durante as alterações de linha
Você pode escrever código para verificar se cada coluna que você deseja validar contém dados que atendem aos requisitos do seu aplicativo. Faça isso definindo a coluna para indicar que ela contém um erro se um valor proposto for inaceitável. Os exemplos a seguir definem um erro de `Quantity` coluna quando a coluna é 0 ou menor. Os manipuladores de eventos de alteração de linha devem se parecer com os exemplos a seguir.

### <a name="to-validate-data-when-a-row-changes-visual-basic"></a>Para validar dados quando uma linha é alterada (Visual Basic)

1. Abra o conjunto de dados no **Designer de Conjunto de Dados**. Para obter mais informações, confira [Passo a passo: Criando um conjunto de um DataSet](walkthrough-creating-a-dataset-with-the-dataset-designer.md)no designer de conjunto de dados.

2. Clique duas vezes na barra de título da tabela que você deseja validar. Essa ação cria automaticamente o <xref:System.Data.DataTable.RowChanging> manipulador <xref:System.Data.DataTable> de eventos do no arquivo de classe parcial do conjunto de um.
...
Cada linha em uma tabela de dados tem <xref:System.Data.DataRow.RowState%2A> uma propriedade que controla o estado atual dessa linha usando os valores <xref:System.Data.DataRowState> na enumeração. Você pode retornar linhas alteradas de um conjunto de dados ou tabela de `GetChanges` data chamando o <xref:System.Data.DataSet> método <xref:System.Data.DataTable>de um ou. Você pode verificar se existem alterações antes de chamar `GetChanges` chamando o <xref:System.Data.DataSet.HasChanges%2A> método de um DataSet.

> [!NOTE]
> Depois de confirmar as alterações em um conjunto de dados ou em uma tabela <xref:System.Data.DataSet.AcceptChanges%2A> data (chamando o `GetChanges` método), o método não retorna nenhum dado. Se seu aplicativo precisar processar linhas alteradas, você deverá processar as alterações antes de chamar `AcceptChanges` o método.
...
- Chame o <xref:System.Data.DataSet.GetChanges%2A> método de um DataSet.
     ...
     O exemplo a seguir cria um novo conjunto `changedRecords` de recursos chamado e o popula com todos os registros alterados de outro `dataSet1`DataSet chamado.
...
- Chame o <xref:System.Data.DataTable.GetChanges%2A> método de uma DataTable.
     ...
     O exemplo a seguir cria uma nova tabela de `changedRecordsTable` dados chamada e a preenche com todos os registros alterados de outra tabela de `dataTable1`dados chamada.
...
- Chame o `GetChanges` método de uma tabela de dados ou de um DataSet <xref:System.Data.DataRowState> e passe um valor de enumeração como um argumento.

     O exemplo a seguir mostra como criar um novo conjunto de `addedRecords` recursos chamado e preenchê-lo somente com os registros que `dataSet1` foram adicionados ao conjunto de um.
...
Quando são feitas alterações nas linhas de dados, o conjunto de registros retém as versões<xref:System.Data.DataRowVersion.Original>original () e<xref:System.Data.DataRowVersion.Current>New () da linha. Por exemplo, antes de chamar `AcceptChanges` o método, seu aplicativo pode acessar as diferentes versões de um registro (conforme definido <xref:System.Data.DataRowVersion> na enumeração) e processar as alterações de acordo.

> [!NOTE]
> Versões diferentes de uma linha existem somente após serem editadas e antes da chamada do `AcceptChanges` método. Depois que `AcceptChanges` o método for chamado, as versões atuais e originais serão as mesmas.

Passar o <xref:System.Data.DataRowVersion> valor junto com o índice de coluna (ou nome de coluna como uma cadeia de caracteres) retorna o valor da versão de linha específica dessa coluna. A coluna alterada é identificada durante <xref:System.Data.DataTable.ColumnChanging> os <xref:System.Data.DataTable.ColumnChanged> eventos e. Esse é um bom momento para inspecionar as diferentes versões de linha para fins de validação. No entanto, se você suspendeu temporariamente as restrições, esses eventos não serão gerados e será necessário identificar de forma programática quais colunas foram alteradas. Você pode fazer isso Iterando pela <xref:System.Data.DataTable.Columns%2A> coleção e comparando os valores diferentes. <xref:System.Data.DataRowVersion>
