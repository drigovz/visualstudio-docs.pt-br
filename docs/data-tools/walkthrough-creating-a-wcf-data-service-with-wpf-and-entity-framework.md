...
- Crie um [!INCLUDE[adonet_edm](../data-tools/includes/adonet_edm_md.md)] que representa a tabela `Customers` no banco de dados Northwind.

- Crie um [!INCLUDE[ss_data_service](../data-tools/includes/ss_data_service_md.md)].
...
Na próxima etapa, você cria um [!INCLUDE[adonet_edm](../data-tools/includes/adonet_edm_md.md)] que representa a tabela `Customers` no banco de dados Northwind.
...
7. Na barra de **endereços** , digite `Customers` no final da URL para **NorthwindCustomers. svc**e escolha a tecla **Enter** .
     ...
     Uma representação XML dos dados na tabela `Customers` é exibida.
...
1. Na barra de menus, escolha **Project** > **Adicionar referência de serviço**.
...
4. No **Gerenciador de soluções**, abra o menu de atalho para o nó **Form1** e escolha **Exibir código** para abrir o editor de código e adicione a seguinte `Imports` ou `Using` instrução na parte superior do arquivo:
...
9. Na barra de menus, escolha **depurar** > **iniciar a depuração** para executar o aplicativo. As informações do cliente são mostradas.
...
3. Abra o menu de atalho para o controle de <xref:System.Windows.Forms.Button>, escolha **Exibir código**e, em seguida, adicione o seguinte código ao manipulador de eventos `Button1_Click`:
...
5. Na barra de menus, escolha **depurar** > **iniciar a depuração** para executar o aplicativo.
