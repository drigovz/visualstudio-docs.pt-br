...
| `--remove <one or more workload or component IDs>` | **Opcional**: uma ou mais cargas de trabalho ou IDs de componente a serem removidas. Para obter mais informações, consulte nossa página [IDs de carga de trabalho e de componente](workload-and-component-ids.md). Você pode repetir esta opção conforme necessário.|
...
| `--norestart` | **Opcional**: se presente, os comandos com `--passive` ou `--quiet` não reiniciarão automaticamente o computador (se necessário).  Isso será ignorado se `--passive` e `--quiet` não forem especificados.  |
