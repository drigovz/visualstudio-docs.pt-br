...
* A propriedade **UseIISExpressByDefault** é adicionada e definida como falsa para que o servidor web que é usado para depurar não seja alterado automaticamente do IIS (Serviços de Informações da Internet) para o IIS Express. O IIS Express é o servidor Web padrão para projetos que são criados com as versões mais recentes das ferramentas.
...
2. No Gerenciador de Soluções, abra o menu de atalho para o nó do projeto, escolha **Propriedades** e, em seguida, escolha a guia **Aplicativo** na caixa de diálogo que aparece.
    ...
    A guia **Aplicativo** exibe a versão de ferramentas que está associada ao projeto. Se for exibida a versão atual das ferramentas do Azure, o projeto já foi atualizado. Se você tiver instalado uma versão mais recente das ferramentas do que a guia mostra, um botão **Atualizar** será exibido.
